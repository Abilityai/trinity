<template>
  ...
  <div class="h-full overflow-y-auto px-4 sm:px-6 lg:px-8 py-4">
    ...
    <!-- Agents List.
         ...
         Edge insets are ordinary item MARGINS (`list-wide:ml-8` on the first cell,
         `list-wide:mr-4` on the last), identical on the header and every row — NOT
    ...
    <!-- The desktop grid switches on the LIST's width, not the window's
         (`list-wide:`, 68rem, in tailwind.config.js). Its non-name tracks need
         about 920px, so behind the systems rail (open on first launch, 224px
         wide) a 1280 window left the `1fr` name track ~70px, and the system
         agent's name disappeared behind its badge. Below the threshold the
         tablet layout renders instead. The query container is this wrapper, not the
         panel root, so the root's fixed-position toast stays viewport-anchored
         whatever containment `container-type` applies. -->
    <div class="[container-type:inline-size] [container-name:agent-list]">
    <div class="flex flex-col gap-y-1.5 list-wide:grid list-wide:grid-cols-[auto_auto_auto_1fr_46px_22rem_180px_auto_auto_auto] list-wide:gap-x-4">
      ...
      <div
        ...
        :class="[
          ...
          'list-wide:grid list-wide:grid-cols-subgrid list-wide:col-span-full list-wide:items-center list-wide:gap-y-1 list-wide:py-3',
      ...
      >
        ...
        <!-- Desktop layout (lg+). `contents` rather than a box: these eleven
             ...
             in for eleven `hidden list-wide:…` cells and no extra box sits between the
             row and its tracks. -->
        <div class="hidden list-wide:contents">
            <!-- Checkbox (track 1 — carries the row's left inset) -->
            <input
              type="checkbox"
              :checked="selectedAgents.includes(agent.name)"
              @change="toggleSelection(agent.name)"
              class="w-4 h-4 list-wide:ml-8 text-blue-600 bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-blue-500 cursor-pointer flex-shrink-0"
            />
            ...
            <CapacityMeter
              ...
              :height="48"
              :width="6"
              class="flex-shrink-0 list-wide:col-start-10 list-wide:row-start-1 list-wide:row-span-2 list-wide:self-stretch list-wide:mr-4"
            />
        </div>

        <!-- Tablet layout (md and up, while the list is narrower than the
