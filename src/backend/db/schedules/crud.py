...
class ScheduleCrudMixin:
    ...
    def _row_to_schedule(row) -> Schedule:
        ...
        return Schedule(
            ...
            # ent#498: Workspace delivery target (graceful default for
            # pre-migration rows, which is also the "no delivery" value).
            ...
            with get_engine().begin() as conn:
                conn.execute(
                    insert(agent_schedules).values(
                        ...
                        # ent#498: already normalized + roster-checked at the
                        # router; None means no Workspace delivery.
        ...
        for key, value in updates.items():
            if key in allowed_fields:
                if key == "enabled":
                    ...
                    value = max(30, min(600, int(value)))
                elif key == "deliver_to_workspace_email":
                    # ent#498: an explicit null OR a blank string clears the
                    # target. Blank is folded to NULL here rather than stored,
                    # so "no delivery" has exactly one representation in the
                    # column the scheduler and the dispatch path both read.
                    value = value.strip().lower() if isinstance(value, str) and value.strip() else None
                values[key] = value
