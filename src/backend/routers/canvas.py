# mcp: canvas.ts (set_canvas / patch_canvas / get_canvas / list_canvases / clear_canvas → /api/agents/{name}/canvas); deliberately NOT exposed (ent#553): PUT .../canvas/{id}/pin — pin is the reader's decision, never the agent's — and POST .../canvas/bulk-delete — an agent's bulk delete is clear_canvas per id under the #918 self-gate, so the human-only verb has no agent caller; and (ent#554) the three share routes — POST .../canvas/{id}/share, GET .../canvas/shares, DELETE .../canvas/shares/{id} — because deciding who OUTSIDE the platform may read a canvas is a grant, not a use, and they refuse an agent principal at the route as well (`_gate_share`), since "no tool exposes it" is a property of the client
