...
def run_static(snapshot: Dict[str, Any], check_ids: List[str]) -> Dict[str, Result]:
    ...
    for cid in check_ids:
        ...
        try:
            ...
            # ent#89: this swallow previously left NO trace anywhere — a check
            # that raises drops out of `_counts` entirely (which counts only
            # `status == "fail"`), so a broken validator reports "healthy" and
            # nothing in the logs says otherwise. Logging it is a zero-behaviour
            # change that makes all ~100 checks' failures observable, and is the
            # instrument for deciding whether to flip this swallow to `fail`
            # platform-wide later.
