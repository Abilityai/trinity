...
Before adding endpoints, services, DB tables, or frontend views, review the Architectural Invariants section in @docs/memory/architecture.md — the always-loaded core, which also carries the Architecture Map naming the on-demand area file that owns each part of the tree (read that file before changing behaviour it documents — a deliberate step, nothing injects it for you). Violations of these patterns will break the system. Run `/validate-architecture` weekly to catch drift. For decisions about new capabilities or significant design choices, also consult `docs/planning/TARGET_ARCHITECTURE.md` — prefer changes that move toward the target, reject changes that move away from it.
...
### 10. Frontend Design System — #1430
Before writing or changing **any** code under `src/frontend/`, read `docs/memory/design-system-contract.md` — the condensed, binding build contract. The full system of record is `docs/memory/design-system.md` (token taxonomy, both-theme rules, primitives catalog, data-loading motion standard, and the 29 UI Construction Principles), with `docs/memory/design-system-reference.html` as the approved visual spec. Non-negotiables: semantic tokens only (no raw Tailwind palette classes, no hardcoded hex), primitives over hand-rolled lookalikes, light and dark both first-class, layout stability (reserve space; no scroll/state resets), bounded viewports for unbounded data. Raw-color usage is ratcheted, and **enforced** (#2605): `tests/unit/rawColorRatchet.spec.js` fails `npm run test:unit` when a file's raw-palette / hex counts grow past `src/frontend/raw-color-baseline.json`, when a file with **no** baseline entry carries any non-gray palette class ("new code starts at zero"), or when an entry sits above reality after a paydown (regenerate so the ratchet keeps biting: `node src/frontend/scripts/scan-raw-colors.mjs src/frontend --baseline src/frontend/raw-color-baseline.json`). It was documented as ratcheted from 2026-07-31 and nothing ran it until #2605, so the baseline was re-frozen there with the increases named. Bare loading gates are ratcheted too (#1927): `tests/unit/loadingGateRatchet.spec.js` fails `npm run test:unit` when a file's count of `v-if="<loading flag>"` gates grows past `src/frontend/loading-gate-baseline.json` — gate on "no data yet" via `utils/loadingState.js::viewState` instead, and regenerate the baseline with `node src/frontend/scripts/scan-loading-gates.mjs src/frontend/src --baseline src/frontend/loading-gate-baseline.json` when you fix one.
...
| `docs/memory/design-system.md` | **Frontend design system — system of record** (#1430): tokens, both-theme rules + dark ink ladder, primitives catalog, data-loading motion standard, 29 UI Construction Principles. Binding contract: `design-system-contract.md` (read before any `src/frontend/` change); approved visual spec: `design-system-reference.html` |
