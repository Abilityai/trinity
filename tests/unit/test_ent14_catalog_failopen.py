...
def env(monkeypatch):
    ...
    _install_settings_stub(
        monkeypatch, settings, github_templates=settings.get_github_templates
    )
    monkeypatch.setattr(
...
def test_the_config_hard_switch_beats_the_db_row(monkeypatch, env):
    ...
    # Through the stub module — its namespace is a copy of the real one, so
    # `SettingsService` is the real class.
    real = sys.modules["services.settings_service"].SettingsService()
