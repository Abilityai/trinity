...
class _FakeClient:
    """Records the path asked for, so the test can assert WHICH agent was asked."""
...
def test_agent_with_a_credential_runs(jc, mode):
    ...
    client = _FakeClient(payload={"agent_name": "a", "auth_mode": mode})
...
def test_not_configured_skips_with_the_allowlisted_reason(jc):
    ...
    client = _FakeClient(payload={"agent_name": "a", "auth_mode": "not_configured"})
...
def test_skip_reason_is_still_allowlisted_by_the_audit(jc):
    ...
    client = _FakeClient(payload={"auth_mode": "not_configured"})
    with pytest.raises(pytest.skip.Exception) as exc:
...
def test_gate_asks_about_the_named_agent(jc):
    ...
    client = _FakeClient(payload={"auth_mode": "subscription"})
    jc.skip_unless_agent_can_answer(client, "agent-b")
    assert client.paths == ["/api/subscriptions/agents/agent-b/auth"]
...
def test_auth_mode_is_read_once_per_agent(jc):
    ...
    client = _FakeClient(payload={"auth_mode": "api_key"})
    for _ in range(4):
        jc.skip_unless_agent_can_answer(client, "same-agent")
    assert len(client.paths) == 1

    # A DIFFERENT agent is still asked about — the cache is per agent, not a
    # single global verdict. J10 has two agents and only the callee matters.
