...
The sweep found six defects. Five remain strict-``xfail`` (F1–F5b): they are
real, recorded rather than patched, because this file's job is to find them and
fixing product code is a separate decision. ``strict=True`` matters — each one
turns into a FAILURE the moment the defect is fixed, so the fix cannot land
...
+---------+--------+--------------------------------------------------------+
| finding | issue  | one-line                                               |
+=========+========+========================================================+
| F1      | ent#399| ``_same_origin`` compares IPv6 literals textually       |
| F2      | ent#398| port ``:0`` normalised three ways along one call path   |
| F3      | ent#397| refusal ``reason`` reverse-engineered from its message  |
| F4      | ent#395| ``remove_endpoint`` can delete two endpoints at once    |
| F5b     | ent#396| a whitespace-only credential clears the stored secret   |
+---------+--------+--------------------------------------------------------+
...
deciding what to work on would never see these five (ent#393 review, I1).
...
# --------------------------------------------------------------------------- #
# D7 / E7 — FINDING F2: port `:0` means three different things in one call path
# --------------------------------------------------------------------------- #
@pytest.mark.xfail(
    strict=True,
    reason=(
        "BUG (finding F2): port `:0` is normalised inconsistently. "
        "`_validate_public_https_url` coalesces it to 443 via `parsed.port or 443` "
        "(0 is falsy), `_pinned_url` drops it and connects to 443, but `_same_origin` "
        "compares it literally as port 0 — so a `:0` endpoint validates, would "
        "connect correctly, and is then permanently refused `card_origin_mismatch` "
        "against any card declaring the ordinary form. Fail-closed. "
        "Tracked as ent#398. See /edge-cases report 2026-08-13."
    ),
)
...
def test_G7_removing_one_endpoint_removes_exactly_one_endpoint(store):
    ...
    # An operator names a third endpoint after alpha's id — nothing forbids it.
    store.upsert_endpoint(alpha["id"], PEER_URL, "gamma-secret")
    assert len(store.list_oss_endpoints()) == 3

    assert store.remove_endpoint(alpha["id"]) is True

    remaining = {r["name"] for r in store.list_oss_endpoints()}
    assert len(remaining) == 2, (
        f"one DELETE removed {3 - len(remaining)} endpoints: {remaining}"
    )
