...
def _gs():
    """#1028: `services/git_service.py` is a package, and it deliberately does
    NOT re-export private collaborators — a name mirrored on both the package
    and its owning module gives one function two monkeypatch targets. Every
    symbol this file reaches (`_GITIGNORE_*`, the two command builders, the
    sweep parser and its tags) lives in `gitignore`, so it names that module.
    """
    try:
