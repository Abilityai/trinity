"""
Unit tests for A2AExposureMixin (ent#157).

The OSS `agent_ownership.a2a_exposed` primitive the public A2A server reads +
enforces. Covers the DB-side toggle, the deleted_at-guarded setter (a
soft-deleted agent must never be flipped into exposed state), and
get_a2a_exposed_agents. Mirrors test_mcp_exposure_mixin.py.
"""
...
def mixin(tmp_db_conn):
    try:
        from db.agent_settings.a2a_exposure import A2AExposureMixin
    except ImportError:
        pytest.skip("backend venv required (no `db.agent_settings` import)")

    class _Wrapper(A2AExposureMixin):
        pass
...
def _insert_agent(conn, name, exposed=None, deleted_at=None):
    conn.execute(
        "INSERT INTO agent_ownership (agent_name, owner_id, created_at, a2a_exposed, deleted_at) "
        "VALUES (?, 1, 'now', ?, ?)",
...
def test_default_false_when_no_row(mixin):
    assert mixin.get_a2a_exposed("ghost") is False
...
def test_default_false_when_column_unset(mixin, tmp_db_conn):
    ...
    tmp_db_conn.commit()
    assert mixin.get_a2a_exposed("a1") is False


def test_true_when_column_is_one(mixin, tmp_db_conn):
    _insert_agent(tmp_db_conn, "a1", exposed=True)
    assert mixin.get_a2a_exposed("a1") is True


# Setter — round-trip
def test_set_true_then_get(mixin, tmp_db_conn):
    _insert_agent(tmp_db_conn, "a1")
    assert mixin.set_a2a_exposed("a1", True) is True
    assert mixin.get_a2a_exposed("a1") is True


def test_set_false_then_get(mixin, tmp_db_conn):
    _insert_agent(tmp_db_conn, "a1", exposed=True)
    assert mixin.set_a2a_exposed("a1", False) is True
    assert mixin.get_a2a_exposed("a1") is False


def test_set_returns_false_when_agent_missing(mixin):
    assert mixin.set_a2a_exposed("ghost", True) is False


def test_set_isolated_between_agents(mixin, tmp_db_conn):
    _insert_agent(tmp_db_conn, "a1")
    _insert_agent(tmp_db_conn, "a2")
    mixin.set_a2a_exposed("a1", True)
    assert mixin.get_a2a_exposed("a1") is True
    assert mixin.get_a2a_exposed("a2") is False


# deleted_at guard — a soft-deleted agent can never be flipped exposed
def test_setter_refuses_soft_deleted_agent(mixin, tmp_db_conn):
    ...
    assert mixin.set_a2a_exposed("gone", True) is False
    row = tmp_db_conn.execute(
        "SELECT a2a_exposed FROM agent_ownership WHERE agent_name='gone'"
    ).fetchone()
    assert row["a2a_exposed"] == 0
...
def test_getter_ignores_soft_deleted_agent(mixin, tmp_db_conn):
    ...
    assert mixin.get_a2a_exposed("gone") is False


# get_a2a_exposed_agents — full-set listing
def test_list_only_exposed_live_agents(mixin, tmp_db_conn):
    ...
    _insert_agent(tmp_db_conn, "gone", exposed=True, deleted_at="2026-01-01T00:00:00Z")
    names = sorted(a["agent_name"] for a in mixin.get_a2a_exposed_agents())
    assert names == ["on1", "on2"]


def test_list_empty_when_none_exposed(mixin, tmp_db_conn):
    _insert_agent(tmp_db_conn, "off1", exposed=False)
    assert mixin.get_a2a_exposed_agents() == []
