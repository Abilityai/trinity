...
class TestWriteCredentialGuard:
    ...
    def test_sync_to_github_blocks_tokenless_with_named_conflict(self, git_service_env):
        ...
        with patch.object(gs.sync, "get_agent_container", MagicMock(return_value=c)), \
             patch.object(gs.sync, "execute_command_in_container",
    ...
    def test_helper_resolvable_credential_counts_as_writable(self, git_service_env):
        ...
        with patch.object(gs.sync, "execute_command_in_container", probe):
            assert _run(gs._agent_can_push("a1", c)) is True
    ...
    def test_helper_probe_fails_open(self, git_service_env):
        ...
        db.get_agent_github_pat.return_value = None
        c = _FakeContainer(["OTHER=1"])
        with patch.object(gs.sync, "execute_command_in_container",
