...
agents. Create-before-rebuild ordering matters for ent#123: build the image
**before** creating tokenless-template agents.

> **Correction (#1809):** as released, v0.8.5 had **no image-drift detection** —
> the "stop/start-with-recreate" step this note originally described did not
> exist. A plain stop/start restarted the OLD container unchanged; only a
> config-drift recreate happened to adopt the new image. Fixed by #1809: a
> **stop, then start** of an agent now detects a rebuilt base image and
> recreates the container (cold start only — a start of a *running* agent never
> image-recreates it). Note the Operating-Room **fleet restart does NOT pick up
> a rebuilt image** (it bypasses the per-agent start path), and the
> `trinity-system` agent adopts it only via an explicit stop/start.
...
- **#1782** (known CI nit, NOT fixed in this release): the frontend-e2e
  ...
  suite is skipped, not run. **Fixed after this release — see #1782.**
