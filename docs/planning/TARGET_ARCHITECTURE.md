...
7. **Does human-interactive chat belong in the queue at all?** — *Under consideration, not decided.* As written, §Coordination Model routes **every** producer into one per-agent FIFO ordered by `queued_at`. Two consequences fall out, both currently unaddressed: (a) **interactive starvation** — a human chat turn queues behind N batch tasks, so the synchronous edge adapter (§Async-First Communication) just times out; (b) **no per-session serialization** — N competing workers can claim two turns of the same Session-tab session concurrently, which is the concurrent `--resume` on one JSONL that today's Redis `session_lock` exists to prevent. The candidate answer is a **scope cut rather than added machinery** (no priority column, no lanes, no message-group/serial-key claim, all of which import their own edge cases): the durable queue carries **autonomous traffic only** (schedules, webhooks, agent-to-agent `chat_with_agent`, loops, channel messages), while **human chat/session turns keep today's direct synchronous path** and today's session lock. Async-first (Principle #3) is preserved where it carries the volume — a failed chat turn has a human watching who retries, so it needs neither durability nor lease re-delivery. Concurrency would then be **one physical budget of N** per agent: interactive turns take a slot on arrival, workers pull batch work only when slots are spare (optionally reserving one slot so long batch turns can't lock chat out) — capacity stays physical with one owner. Multi-user chat is unaffected either way: sessions are per-user with their own JSONL, so users never serialize against each other. Decide before the #946 pilot, which routes `chat_with_agent` — an interactive `kind` — through the queue first. Note also that under `replica_count > 1` session JSONLs are single-container state, so sessions need affinity or shared storage regardless of this decision (see open question 5 / #927).
