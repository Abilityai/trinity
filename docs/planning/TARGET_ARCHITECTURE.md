...
Crash taxonomy, all recovered by that one move: agent/container/OOM death mid-turn → lease expires → re-queued; backend restart → in-flight long-polls drop, agents re-poll on reconnect, committed queue rows intact; container recreate (e.g. subscription auto-switch) → the queue lives in the backend, so at most the active turn is lost, recovered by lease expiry (and upgraded to a clean drain by a pre-recreate "stop pulling, finish in-flight" handshake). The re-delivery cap **shipped in Phase 3 (#1550)**: `schedule_executions.redelivery_count` + `MAX_REDELIVERY` (default 3; raw attempts, no reset-on-progress — partial progress is invisible while trace fidelity is open), enforced by the lease reaper (`services/lease_reaper_service.py`) — under the cap the SAME row re-queues (`execution_id` preserved), at the cap the task is poison-parked `FAILED(poison_lease)` with an alert-first, idempotent operator-queue item (distinct from #678's `retry_count` reader-race auto-retry). **(v2)** This park is *doubly* load-bearing: it is also the landing spot for the irreversible-and-un-confineable effect class (§"Re-Delivery and Side-Effect Recovery") — the async contract surface (platform prompt + agent guide: fire-and-park, ask-before-irreversible, derived request ids) shipped with **#1402**, and the pull-path *delivery* of that prompt shipped with **#1629** (fix #1633 — `services/pull_coordination_service.py` composes the platform prompt on the claim path, fail-open, and injects the re-delivery banner). Default-on for effect-bearing agents is still gated on trace fidelity (#548/#333), `prior_trace` injection (#1401), and fail-closed `execution_id` injection. SUB-003 parity on the pull sink is **no longer a gate item either**: `apply_task_result` carried every other CAS-won terminal hook (#1578 emit, #1804 activity close) and not the subscription switch, so piloting a subscription-backed agent would have silently removed SUB-003 from it — a quota or credential refusal recorded no failure event, skip-listed nothing, and left the agent on the subscription that had just refused it. **#2643** wires it on the CAS-won branch, mapping the worker's `billing`/`auth` codes onto SUB-003's `rate_limit`/`auth` kinds through an allowlist and delegating to `handle_subscription_failure`, which owns the event write and the #799 per-agent lock. It re-delivers nothing — re-delivery stays the lease reaper's decision under the #1085 governor's pause. Operator-queue flooding caps are **no longer a gate item**: #1632 bounded the agent-authored ingestion seam, and **#1677** closed the platform-emitter residual that bypassed those caps — the one agent-influenceable platform emitter now routes through a per-(agent, type) pending-depth budget, genuinely platform-driven emitters stay direct and unthrottled, and an AST caller-parity guard forces every future emitter to be classified at CI rather than remembered.
...
2a. **Side-effect recovery** (issues #1401, #1402, #1084, **reframed in v2**): exactly-once external effects are impossible (two-generals), so the platform does not attempt them — it synthesizes recovery = **at-least-once delivery + retry-with-prior-trace + deterministic tool-side gates on capability-confined irreversible rails.** The remaining work, now with tracking:
   ...
   2. **Async operator-queue human-gate lever** — the `MAX_REDELIVERY` cap + operator-queue park that catches poison tasks *and* irreversible-un-confineable effects; **no synchronous user gates** (agents fire-and-park). **→ #1402 SHIPPED** (mechanism in Phase 3 #1550: cap + reaper + alert-first poison-park; contract surface — prompt rewrite, task-mode carve-out, derived request ids, agent-guide section — in the #1402 PR; bounds the lease-reaper #429). Pull-path prompt delivery + re-delivery banner **→ #1629 SHIPPED** (fix #1633, on dev). Residual: respond→re-trigger dispatch **→ #1630**; `(agent_name, id)` queue-key scoping **→ #1631**; queue-create flooding caps **→ #1632 SHIPPED** (agent-authored ingestion seam) + **#1677** (platform-emitter residual — the emitters that bypassed #1632's caps are now classified by influence, not by caller location: the agent-influenceable one is budgeted per (agent, type), the platform-only ones stay direct, and an AST caller-parity guard fails CI on an unclassified new emitter) — **no longer blocking default-ON**.
   ...
   **#1084 no longer blocks pull default-on.** Gating the *effect* not the *agent* lets read/analysis-only + reversible + confined-irreversible agents default on immediately; only irreversible-and-un-confineable effects wait.
...
7. **Does human-interactive chat belong in the queue at all?** — **DECIDED 2026-09-16 (@obasilakis): yes — the edge adapter, not the scope cut.** The durable queue carries **all** traffic, interactive included, and the synchronous push dispatch path is deleted once the two supporting pieces land. The scope-cut alternative (queue carries autonomous traffic only; human chat keeps today's direct path) is **rejected**, because it preserves a second set of books permanently: two independent records of who is busy — rows in the database and a counter in Redis — which is the drift class three of the four slot/backlog canary invariants exist solely to detect. One queue means one owner column with an expiry, and those detectors have nothing left to detect. Measured scale: ~6,000 lines of push-side coordination machinery against ~970 lines of pull code already written; an honest retirement figure is 3,000–4,000 lines plus ~600 lines of reconciliation canary. The two consequences this question raised are addressed rather than avoided. (a) **Interactive starvation** → **#2842**. The premise was overstated: push does *not* avoid the wait — human chat already dispatches with `overflow_policy="queue_in_memory"`, a bounded Redis FIFO, so at capacity a person waits for a running turn to end regardless (measured on `eu2`: agent turns average 339–468s, max 1931s, so today's worst case is already ~32 minutes). What push avoids is waiting behind *queued batch work*. Priority ordering therefore restores **parity** with push; a reserved worker would be strictly better and is deferred to #2842 with its capacity cost — one of N workers held idle — stated explicitly. (b) **No per-session serialization** → **#2843**. The requirement is mutual exclusion per conversation — two turns of one conversation never run at once — served by **any** free worker. It is explicitly **not** worker affinity: all N workers share one container filesystem, so there is no file-access reason to pin, and pinning would make a person wait on a busy worker while others idle, defeating (a). Affinity does become necessary under `replica_count > 1`, where session JSONLs are single-container state (open question 5 / #927) — out of scope here. Concurrency remains **one physical budget of N** per agent. Multi-user chat is unaffected either way: sessions are per-user with their own JSONL, so users never serialize against each other.
