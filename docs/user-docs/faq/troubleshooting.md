...
Check the loop's stop reason in the **Loops** tab — loops have several intentional early exits. `stop signal matched` means the agent's response contained your sentinel substring, which is until-mode working as designed. `budget exhausted` means the total cost met your `max_cost_usd` at a run boundary; a deadline (`max_duration_seconds`) similarly ends the loop between runs. A loop also stops for no progress: if several consecutive runs return an identical response (default threshold 3 for new loops; set `no_progress_threshold: 0` to disable), Trinity assumes a doom loop and stops it. Finally, by default loops are fail-fast (`on_failure: abort`) — the first failed iteration ends the loop with status `failed` — but you can set `on_failure: continue` to tolerate failed iterations and keep going, bounded by `max_consecutive_failures` (default 3) and finishing as `completed_with_errors` if any iterations were tolerated. A backend restart marks in-flight loops `interrupted` with no auto-resume; start a new loop. See [Agent Loops](../automation/agent-loops.md).
