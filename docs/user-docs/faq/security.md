...
Only an install with **no admin account** has an open window. On a normal install `start.sh` refuses to start while `ADMIN_PASSWORD` is blank, so the admin exists before you ever open a browser and the first-run form never appears. The unauthenticated "Create your admin account" form shows only when the backend came up with no admin — a bare `docker compose up`, a hand-rolled backend, or a marketplace one-click droplet, which is in this state on purpose so you can create its admin account in the browser with no terminal — and in that state whoever submits it first owns the instance, so keep such an instance behind a VPN, tunnel, or firewall until the account exists, or open the droplet and create the account as soon as it is up (supplying a password when you create the droplet closes the window altogether). The endpoint refuses with 403 whenever a usable admin already exists, whatever the setup flag says, so it cannot be used to take over an install that is already configured. See [Setup](../getting-started/setup.md).
