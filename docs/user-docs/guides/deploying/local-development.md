...
Log in with username `admin` and the `ADMIN_PASSWORD` from `.env`. There is no setup screen: setting `ADMIN_PASSWORD` provisions the admin account at boot. The Dashboard then opens the first-run setup sequence — **Connect Claude** (a subscription token or API key) is its one required step; the rest can be skipped and picked up later from **Settings → General → Re-run setup**. See [First-Time Setup](../../getting-started/setup.md).
