...
- **The ask.** The **Usage sharing** step of first-run setup — the overlay that opens over the Dashboard for an admin who has not yet decided (see [Setup](../getting-started/setup.md#your-first-dashboard)). The step is headed **Help improve Trinity** with an **Off by default** badge, and **See what would be sent** expands the exact payload. Two choices decide it: **Share anonymous usage** (turns sharing on, with the last 30 days of history) and **Don't ask again** (a once-per-install marker that stops the ask on every browser and device; audit-logged). **Skip — later in Settings → General** leaves it undecided for this browser only — nothing reopens the overlay for a skipped step, but another browser or device is still asked. After the instance's first successful scheduled or webhook run the step is headed **Your first scheduled run just completed** instead, once per browser; that alone never reopens a setup you closed — you see it when the overlay opens for another reason or is re-run from **Settings → General**. Consenting, **Don't ask again**, or a config hard-disable all silence it permanently. A **Not now** given to the earlier Finish-setup card counts as a skip while its 14 days last.
