...
| T3.7 | Canary E-01 fire on a legitimately-running pull turn | P1 | ✅ **CLOSED by #1990** — fixed via lease-exclusion, not accepted. Not merely a re-delivery transient: `claim_next_queued` stamps the lease at `started_at + timeout + SLOT_TTL_BUFFER`, the **identical** window, so E-01 fired at the instant the reaper's recovery window opened and stayed red until its next sweep, every re-delivery, per execution, at **critical**. A NULL-lease control of the same age still fires. |
...
~~canary lease-awareness E-01 (§3 T3.7)~~ ✅ **closed by #1990** — same exclusion, on the invariant with the
worst overlap: the lease is stamped at exactly `timeout + SLOT_TTL_BUFFER`, so E-01 fired **critical** the
moment the lease-reaper became eligible and stayed red until its next sweep. Canary lease-awareness is now
complete: S-01, E-05 and E-01 exclude leased rows; E-02 deliberately does not (a terminal→non-terminal
reversal is corruption regardless of ownership, and pull is the more exposed path) ·
