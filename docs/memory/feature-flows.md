...
|------|-----|---------|------|
| 2026-06-26 | #679, #1333 | Cancelled propagated as a first-class third terminal outcome end-to-end (agent `_terminated` marker + relabel, neutral dispatch-breaker finish, backend 3-way status map with AUTH trust-boundary guard, consumer non-delivery incl. paid no-settle money-bug fix) | [execution-termination.md](feature-flows/execution-termination.md) |
| 2026-06-26 | #1112, #1340 | Unified Chat + Session into one Chat tab with a session-mode toggle (default ON) | [session-tab.md](feature-flows/session-tab.md), [authenticated-chat-tab.md](feature-flows/authenticated-chat-tab.md) |
| 2026-06-26 | trinity-enterprise#17, #1317 | feat(access): Access tab — read-only roster of Trinity **operators** (platform users) per agent, resolving each `agent_sharing` allow-list email against `users` (active operator vs pending invite); add/remove reuse the existing `/share` endpoints (no new write path). | [access-tab.md](feature-flows/access-tab.md) |
...
| Access Tab (Operators) | [access-tab.md](feature-flows/access-tab.md) | Read-only operator roster per agent — `agent_sharing` emails resolved to active operators vs pending invites (trinity-enterprise#17) |
