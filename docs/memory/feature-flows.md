...
| Date | ID | Change | Flow |
|------|-----|--------|------|
| 2026-07-26 | #1759 | fix(agents): an **absent `local:` template now fails loudly**. `_resolve_local_template` had no `else` — a well-formed but unresolvable `local:<name>` returned empty `template_data` and the agent was created anyway (HTTP 200, blank container, no warning); only *malformed* names 400'd. Now 400 `LOCAL_TEMPLATE_NOT_FOUND` **before any side effect** (same pre-side-effect band as `FORK_REQUIRES_GITHUB_TEMPLATE`; `create_agent_internal` gains zero lines, #1484), plus 400 `LOCAL_TEMPLATE_INVALID` for an empty/non-mapping/unparseable `template.yaml` (the identical blank-agent outcome via the old bare `except Exception: logger.warning`). ONE identical message whichever root missed, with no filesystem path or root name — deploy-local templates are named after AGENT names (#186 adjacency). The curated root gains a repo-relative fallback (`parents[4]`, hand-rolled — importing `template_service` would be satisfied by the #1484 harness MagicMock) so the gate is not inert wherever the suite runs; **both** seams take it, and the `/template` bind base becomes `os.getenv(...) or _default_host_templates_base()` (an empty `HOST_TEMPLATES_PATH` made `Path("")/name` an empty NAMED VOLUME — the same bug class one seam over). Ships `config/agent-templates/default/` (hidden, deliberately no `resources:` — it would neutralise ent#125's `cpu: "3"` failure vector) so the `local:default` id the docs and live-server suites already used resolves. `export_manifest`'s unreachable `local:business-assistant` `.get` default fixed with `or` (pre-existing broken round-trip for Blank Agents, not a regression). Manifest deploys surface it per agent via ent#125 `failed[]`. Closes the follow-up left open by #843. | [template-processing.md](feature-flows/template-processing.md), [system-manifest.md](feature-flows/system-manifest.md) |
...
| Operating Room | [operating-room.md](feature-flows/operating-room.md) | Unified operator command center: queue, notifications, resolved (OPS-001); async fire-and-park contract + derived request ids + lease-reaper poison-park items (#1402); agent-authored ingestion depth/rate/size caps + reserved-id guard + leader lock (#1632) |
