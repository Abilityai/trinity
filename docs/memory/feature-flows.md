...
| 2026-07-30 | ent#109 | fix(lifecycle): **one owner for git env across both container-rebuild paths**. `recreate_container_with_updated_config` seeds `env_vars` from the **old container** and re-derived only `GITHUB_PAT` — replaying whatever `GITHUB_REPO`/`GIT_SYNC_*` each container carried; the derivation lived only on the other path (`_apply_persisted_auth_env` → `recreate_missing_container`). Its one production caller is `start_agent_internal`, fired by nine config-drift predicates **and base-image drift at cold start**, so a base-image rebuild arms the replay fleet-wide. New `_apply_git_env_from_db(..., *, pat_gate)` owns both. The **PAT gate is a required per-call-site parameter, never a default**: `per_agent_only` (config drift) keeps #211's opt-in verbatim so a global-only platform PAT is never injected into a previously-tokenless container, `effective` (rebuild-from-nothing) uses the 2-tier resolver — a verbatim lift would have shared the broad gate, `configure_push_remote` would clear the push blackhole, and a tokenless agent (Cornelius) could push a private KB to the shared public upstream (the ent#162 class). An **AST guard pins the writer SET**, not just the two known sites (`{recreate_container_with_updated_config: per_agent_only, _apply_persisted_auth_env: effective}` exactly), so a *third* writer on any container-seeded path fails CI — ent#109's bug WAS two writers with one wrong. Plus **set-or-clear** over `_GIT_ENV_KEYS` (a deleted `agent_git_config` row pops the whole set — the per-agent PAT is a column *on* that row), **correct-never-introduce** on the config-drift path (repo-gating the block while per-agent-gating the PAT would hand startup.sh `GIT_SYNC_ENABLED=true` with no token for an agent bound via `POST /{agent}/git/initialize`, and the restart branch would then scrub its only credential), and `GIT_SYNC_AUTO` as **DB flag OR baked env, derive-only** — `crud.py`'s two creation writers genuinely disagree (`and not config.ephemeral` inside a swallowing `try/except` on the DB side only) so DB-only derivation would silently stop auto-push, but the row is **never written back**: "baked true / DB 0" is also exactly an owner's explicit `PUT .../auto-sync {enabled:false}`, and that endpoint is `OwnedAgentByName` while `POST .../start` is `AuthorizedAgentByName`. Tests: `test_ent109_git_env_seam.py` (22), each behaviour proved to have teeth by mutation. | [agent-lifecycle.md](feature-flows/agent-lifecycle.md), [github-sync.md](feature-flows/github-sync.md), [git-sync-health.md](feature-flows/git-sync-health.md) |
