...
| Git Sync Health | [git-sync-health.md](feature-flows/git-sync-health.md) | Auto-sync heartbeat, dual ahead/behind, dashboard dot, `/api/fleet/sync-audit`, creation-time canonical `.gitignore` seed (#2069, readiness-gated), lock-free single-flighted status read + observe-only stuck-lock report (#2742) |
