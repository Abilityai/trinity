...
| 2026-09-13 | ent#454 | feat(systems): **deploy gained its inverse** — removing a manifest-deployed fleet meant `DELETE /api/agents/{name}` once per member by hand (eleven times for the bundled flagship manifest), which is why the install UI warned *"there is no un-deploy"* and why the duplicate-deploy trap had no recovery path. The verb is an **entitlement-gated module** (private tree); this repo gained the edition-agnostic half. `system_member_names` keeps its signature and becomes a thin wrapper over a new `system_membership`, which additionally returns `tags_readable` and per-member `evidence` — the two facts the flat name list discarded and only a DESTRUCTIVE consumer needs. That is what lets teardown **refuse** where the predicate **degrades**: #2373 ranks a tag-read failure toward the raw name prefix because for `restart` over-capture restarts one extra container and logs it while under-capture restarts a subset and reports success — a verb that deletes inverts the cheap half, so it ranks the two errors at its own call site instead of forking #2373's ONE predicate into a fourth rule. Membership is pinned as Hypothesis **properties** (`members ⊇ raw prefix match` when tags are unreadable, `tagged ⊆ members` when they are readable, wrapper equivalence) rather than cases, because that fallback was wrong three times and each wrong rule passed the case test written for its predecessor; one counterexample it found is kept as an honest asymmetry (an empty roster never reaches the tag read, so it reports `tags_readable: True` — refusing there would be a 503 where 404 is the truth). `SYSTEM_VIEW_AUTO_DESCRIPTION` replaces an inline f-string: nothing persists a system↔view link, so that description IS the identifier for the view a deploy auto-created, and a reader must not restate it. Third surface is a **license-blind** MCP proxy (`teardown_system`, `dry_run` defaulting to `true` — the opposite of `deploy_system`) that keeps 404/403/503 as three distinct operator situations and never throws; it reads `ApiError.body` rather than regexing `.message`, which that field exists for. UI is a gated "Remove a deployed system" panel whose preview is a **checklist** — a `prefix`-evidence member may belong to a sibling system, the server cannot tell, so the badge plus the opt-out is the over-capture escape hatch — with Remove gated on four conditions and `ManifestPreview`'s "no un-deploy" warning now tracking the **capability** rather than being retired on every edition. | [system-manifest.md](feature-flows/system-manifest.md), [library-page.md](feature-flows/library-page.md), [mcp-orchestration.md](feature-flows/mcp-orchestration.md) |
