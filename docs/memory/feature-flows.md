...
| Install Provenance & Hardening Guide | [install-provenance.md](feature-flows/install-provenance.md) | Write-once `TRINITY_INSTALL_SOURCE` → `system_settings.install_source` gating a two-stage (domain, then Cloudflare Tunnel) first-run card on marketplace and `do-script` installs only (`hardening_guide_eligible`); per-stage dismissal; Caddy's `/api/public/tls-allowed` exact-host `ask` gate makes the domain step a Settings field (#2380) |
