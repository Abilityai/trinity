...
| 2026-09-13 | ent#615 | fix(git): **the fleet PAT stops being readable inside every agent container**. Trinity persisted every remote as `<scheme>://oauth2:<PAT>@<host>/…`, so the fleet-wide token was at rest in `.git/config` on the workspace volume AND — because git expands the stored URL into `git-remote-https`'s argv — in the process table ~2x/min from the 60s sync-health poll alone, from where `ps` → the orphan sweep's reaped-cmdline log → Vector → the logs API carried it into **another agent's context** (ent#292 closed that last hop and was rated P0; this closes the cause). A git **credential helper** replaces it: git speaks the credential protocol over stdin, which never reaches argv and is never persisted, and it needs zero changes at the ~42 raw `subprocess.run(["git", …])` sites. Three properties each cost a design iteration — it is registered as **`trinity`**, never as its filename (git prepends `git-credential-`, so the filename resolves to a command that does not exist and the helper *silently never runs*: a fleet-wide fetch/push outage no source-only CI can see); registered **unscoped** with an exact, port-inclusive host check inside, because `TRINITY_GIT_BASE_URL` is a runtime value a build-time `credential.<base>.helper` could not know; and its ladder is `.env` → baked env → harvest file, the **inverse** of `startup.sh`'s, because `startup.sh` runs at boot where baked env is the freshly-recreated truth while the helper runs in steady state where `Config.Env` is immutable without a recreate — baked-env-first makes a global rotation (#1967) a silent no-op until the old token is revoked. **Four** producers, not the two first found: `_git_remote_url` (deleted), `startup.sh`'s CLONE_URL branch, `template_service.clone_github_repo` (deleted — dead, and it put a token URL on **argv** on the backend host) and `skill_service._authenticated_url`, which was LIVE and left the platform PAT at rest in `/data/skills-library/*/.git/config` on the `~/trinity-data` **host bind mount**. Standing rule everywhere: **never strip a credential you have not already replaced** — the sweep proves the helper resolves **by exit code** (`git credential fill` prints the credential on stdout and the sweep's output crosses the `docker exec` boundary into the platform log), harvests only for an agent with no other source, and refuses-and-reports otherwise; `startup.sh`'s per-restart rewrite becomes conditional, because an agent bound via `POST /{agent}/git/initialize` has its only credential in that URL and an unconditional rewrite would destroy it at container start, before any sweep could run. The harvest lands in `.trinity/git-credential` (0600), **not** `.env` as `GITHUB_PAT` — that name is exported as `GH_TOKEN`/`GITHUB_TOKEN` and gates the ent#123 push blackhole, so writing it would be the ent#162 **grant** fleet-wide. Fleet reach is a leader-locked **boot one-shot** plus the `start_agent_internal` hook (**without** #2069's `auto_sync_enabled` gate) — no new recurring service, because nothing produces a token URL any more. `PROTECTED_KEYS` gains the `GIT_CONFIG_KEY_*`/`VALUE_*` **slots** (the count was already guarded, the payload was not) plus `GIT_CONFIG_NOSYSTEM`, which would otherwise disable the `/etc/gitconfig` the helper is registered in. NOT closed, stated rather than discovered: an agent reading its own credential, and blast radius — root ownership is integrity, not confidentiality (ent#558/AAuth). | [github-sync.md](feature-flows/github-sync.md), [github-repo-initialization.md](feature-flows/github-repo-initialization.md), [credential-injection.md](feature-flows/credential-injection.md) |
