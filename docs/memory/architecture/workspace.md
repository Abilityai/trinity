...
(`_apply_readiness_gate`, beside the #454 pre-check and sharing `_record_gate_skip`) asks
...
`changed_by = rollout:ent#689`, insert-if-absent, for every agent with a live seat brief at
deploy. `effective_readiness` reports that stamp as `source: "rollout"` with no person; the card's
`brief_held` says a seat brief is paused.
