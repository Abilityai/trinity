...
| GET | `/.well-known/aauth-{agent,jwks,resource}.json` | AAuth discovery documents (ent#623 prototype, flag OFF → uniform 404). Unauthenticated by protocol, per-IP limited (`routers/aauth.py`) |
| POST | `/a2a/{name}` (AAuth) | ent#623 prototype: a request carrying `Signature-Key` while the flag is on is verified as an AAuth agent (RFC 9421 + `aa-agent+jwt`) instead of a bearer key — 401 problem+json before the body is parsed, 403 unless the identity is explicitly allow-listed (fail-closed), no Trinity user attached. Bearer requests on the same route are unchanged (§32.6) |
