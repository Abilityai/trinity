...
| POST | `/api/agents/{name}/pipeline-state/changed` | **Pipeline-state change notice** (ent#533) — the agent server says one of its own `~/.trinity/pipeline-state/` files changed. Same auth as the heartbeat above (the agent's OWN agent-scoped MCP key; `authorize_heartbeat`, so a user/system/connector/other-agent key is 403), never the internal secret and never an admin gate. Body is `{pipeline_id, instance_id, stage?}`; ids are grammar-checked (422) before anything is published and `stage` is normalised to `null` rather than rejected. Coalesced 20 notices / 10 s per agent → `{ok, published}` (a refused notice is a **200 with `published: false`**, not a 429 — a burst is the agent's normal and the 12 s poll covers it). Publishes the thin `pipeline_state_changed` trigger and bumps the Work read's cache generation. `# mcp: none` |
