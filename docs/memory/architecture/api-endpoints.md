...
| GET | `/api/agents/{name}/metrics` | The agent's declared metrics joined to their RECORDED points, with freshness (ent#479) — the ONE read of a business number. Same URL as the `metrics.json` proxy it replaces; **store-only, no container**, so a STOPPED agent answers identically (the proxy returned "Agent must be running", making every number vanish exactly when an operator wanted its last value). `AuthorizedAgentByName` (uniform 404) **then** the agent self-gate — an agent key reads only its own numbers (cross-agent is ent#80's grant); 240/min per agent. `window ∈ {auto,24h,7d,30d,90d}` + `since`/`until`; `auto` = `max(24h, 12 x cadence)` capped at 90d, because a cadence spans 60s–1y and a fixed 24h shows a weekly metric four points. Bucketed by default (≤120/series); raw `points` only on the single-`metric=` path, truncation keeping the NEWEST. 422 `window_invalid` / `metric_undeclared` (a retired name names `include_retired`); 503 `metric_store_unavailable` + `Retry-After`. NOT 404 for an undeclared metric: the MCP classifier reads 404 as `not_authorized` per the #186 convention, which would tell an agent it lacks access to itself. Echoes the persisted D-010 `metrics.json`-superseded finding with `findings_evaluated_at`, so an empty list before the first compat run reads as *not evaluated* rather than *clean* |
| GET | `/api/agents/{name}/objectives` | The agent's objectives joined to its metrics (ent#666) — target, actual, freshness and the gap, computed in ONE place so the role card, the project hub and proactivity cannot disagree. Gate order is `/metrics`'s verbatim: `AuthorizedAgentByName` (uniform 404) → agent self-gate (403; an agent key reads only its own) → limiter on the VALIDATED name. **Its own rate knob** `OBJECTIVES_READ_RATE_LIMIT` (default 60/min, a quarter of `/metrics`) because unlike that store-only read this one drives the CONTAINER: a directory listing plus up to 100 small file reads through the agent door, ≤ 2 in flight, aborting to `unavailable: agent_unreachable` on the first typed transport failure rather than driving open the circuit chat rides on. Not store-only by design — files are truth and they live in the container (E7/E13), so a stopped agent answers `unavailable: agent_stopped` with copy naming the fix, never a cached number. `gap.status ∈ {behind, on_target, ahead, off_target, not_computable}` is POSITION relative to target given direction, never pace. 503 `metric_store_unavailable` + `Retry-After: 30` is the only non-2xx below the gates — every other failure is a NAMED field on a 200 (`unavailable`, `source.*`, `findings[]`), because "this agent is stopped" is an answer, not an error. `response_model=ObjectiveJoinRead` with a key-parity test, so an additive service field fails the build instead of being silently filtered out |
| GET | `/api/agents/{name}/metrics/definitions` | The agent's DECLARED metric registry (ent#477) — what `template.yaml metrics:` declares, as the backend reconciled it, NOT what it has measured. `AuthorizedAgentByName`; `?include_retired` serves definitions the template no longer declares (kept so points stored under their name stay interpretable). Answers on a STOPPED agent (it is a DB read). Carries `declared` + a next-action `message` for the empty state, `type_conflict` for a refused type change, and the retention/cap `policy` — live values resolved through the settings chain, `enforced: true` and a `source` per knob since ent#478 |
| POST | `/api/agents/{name}/metrics/points` | Record a batch of observations of the agent's DECLARED metrics (ent#478) — the ONLY metric write path. `routers/metric_points.py`, registered before the agents router's single-segment routes. `AuthorizedAgent` + the #918 self-gate (an agent key records only as itself), then rate limit → size (2 MiB; a point cap is not a size cap — 1000 points x 10 dims x 128 chars is ~1.4 MB of legal input) → batch idempotency → validate → daily cap. **All-or-nothing**: 422 `{reason: "invalid_points", errors: [{index, metric, code, message, hint?}]}` with the frozen reason-code list (requirements §48.4). 429 splits into per-minute `rate_limited` and `daily_point_cap_exceeded` (with `Retry-After` to the next UTC midnight), because the remedies differ. DB failures are CLASSIFIED — connectivity is 503 + `Retry-After`, a content error is a non-retryable 500, since an agent told to retry a permanently unstorable batch retries forever. Every non-2xx exit after the idempotency claim goes through one `_reject` choke point that releases it, or one bad batch wedges the caller's key for 24 h. Replay returns the first snapshot with `X-Idempotent-Replay`; an in-flight duplicate is 409 |
...
| GET | `/api/public/tls-allowed` | **Caddy's on-demand-TLS gate** (#2380), unauthenticated because Caddy holds no Trinity credential and calls it mid-handshake. Answers 2xx only for the exact host of the saved `public_chat_url`, parsed not substring-matched, so `evil-example.com` cannot ride on `example.com`. This is what makes adding a domain a Settings field rather than a root shell on the host — Trinity is containerised and cannot rewrite a Caddyfile, so Caddy asks it instead and no privilege moves. Fails CLOSED on an unset URL or a failed read: `on_demand` without a working gate turns the instance into a certificate requester for anyone who points DNS at it, until the ACME account is rate-limited and the operator's own renewals fail |
...
| POST | `/api/enterprise/client-portal/agents/{agent}/sessions/main/reset` | **Reset Main** (ent#523) — archive the pair's pinned Main chat and mint a fresh one, in one transaction. Roster-scoped (uniform 404). "Starts cold" needs no second reset primitive: the new row carries no `cached_claude_session_id` and the turn engine resumes only on a cached id, so `routers/sessions.py::reset_session_memory` is untouched — clearing a cache and retiring a thread are different verbs. The archive keeps its own cached id and title and stays an ordinary past chat; one `role='system'` line in the new Main names it. Named 409s: `turn_in_flight` (retiring a thread mid-turn lands the reply where only a search would find it, billed either way) and `reset_raced`. Resetting an untouched Main is a no-op reported as `archived_session_id: null`. **No confirmation** (operator, 2026-09-06) — nothing is lost. Rate-limited 10/60s per viewer |
| GET | `/api/enterprise/client-portal/work?agents=a,b&chat_id=` | **The Work read** (ent#525) — the participants' in-flight rows + a bounded 30-day history + the open chat's delegated children (found by `source_channel_chat_id`, since a child's `agent_name` is the delegate), projected for the caller (`client_portal/work/`). **Platform door only**: a portal token gets a uniform 404 before any read (ent#78). Roster-scoped by set membership (off-roster names dropped, never answered); every off-roster name on the payload masked to `null`; steps via the #919 pipeline files, best effort, three-state |
| GET | `/api/enterprise/client-portal/agents/{name}/uploads/{filename}` | **Read back one of your own uploads** (#2582). Roster-gated (uniform 404); the filename must survive `_safe_filename` unchanged or it is the SAME uniform 404, so a traversal attempt is indistinguishable from a miss. Reads the per-client inbox via `agent_shared_files_service.extract_from_agent`; serves `Content-Disposition: attachment` + `nosniff` + `Cache-Control: private, no-store`. 409 on a stopped agent, 502 on no container — deliberately consistent with `_read_inbox` returning `[]` for one. Two-tier limiter `PORTAL_FILE_{BURST,HOURLY}_LIMIT`; audit `portal_upload_download` |
| DELETE | `/api/enterprise/client-portal/agents/{name}/uploads/{filename}` | **Delete your own upload** (ent#548) — `rm -f --` in the agent container's inbox, idempotent 204. Same gates, same uniform 404, its own looser limiter (an `rm` is cheap, a `get_archive` is not); audit `portal_upload_delete` |
| DELETE | `/api/enterprise/client-portal/agents/{name}/documents/{file_id}?scope=me\|everyone` | **Remove an agent-shared file from your list, or revoke it** (ent#548) → 204. `scope=me` (default) writes a `portal_file_dismissals` row — per-viewer, the share untouched, and the `file_id` is deliberately NOT validated (an existence oracle over every share id; the row cap bounds the write instead, exactly as `set_chat_star` resolved it). `scope=everyone` needs `portal_owns_agent` — the SAME membership the roster card renders — and soft-revokes through `db.revoke_agent_shared_file`; **access-first** (roster 404 → ownership 403 → row lookup 404), never existence-then-access (Invariant #8). Unlike the sibling `DELETE /api/agents/{name}/shared-files/{id}`, which is idempotent-204 for a missing id, this returns **404** for enumeration-uniformity on an external surface. Audit `portal_share_dismiss` / `portal_share_revoke` |
...
| GET/PUT | `/api/agents/{name}/operator-resume` | Per-agent respond→resume opt-in (ent#329). GET any accessor; PUT **owner-only and human-only** (`reject_agent_principal` — an agent-scoped key resolves to its owner, so without it an agent could switch on its own paid wake-ups; enabling means answers may now spend, and the bill is the owner's). Default OFF — see [Respond → Resume](#respond--resume-dispatch-ent329) |
...
| GET | `/api/settings/feature-flags` | Public-safe UI gating flags (any auth user): `session_tab_enabled`, `voice_available` (`VOICE_ENABLED && ` the Gemini key — Settings → env, resolved per request since ent#582), `voip_available` (#1056), `brain_orb_available` (runtime-resolved: `system_settings` override → `BRAIN_ORB_ENABLED` env opt-in → OFF; gates the Brain Orb page — #58/#85), `brain_orb_voice_available` (`base && voice && GEMINI_API_KEY`, all but the key runtime-resolved, default OFF; gates the client-held voice tile — #60/#85), `mcp_agent_chat_pull_enabled` (#946 observability-only; routing gate is the MCP server's own `MCP_AGENT_CHAT_PULL_ENABLED`, default OFF), `redelivery_governor_enabled` (#1085 observability-only; default OFF), `canary_enabled` (#2217 observability-only — whether the canary harness is enabled; last-cycle/stale/sink detail stays admin-only on `GET /api/canary/status`; default OFF), `tts_available` (ent#117 — an ElevenLabs key resolves via stored setting → `ELEVENLABS_API_KEY` env; gates the voice config UI + `send_voice_reply`), `a2a_outbound_available` (#736 — the outbound-A2A kill switch: `system_settings` → `A2A_OUTBOUND_ENABLED` env → **OFF**; both call routes 404 when off, so this is observability/UI gating, not the enforcement), `install_source` / `marketplace_install` / `hardening_guide_eligible` / `install_tls_posture` (#2380 — how this instance was installed, whether it came from a vendor listing, the gate for the first-run overlay's `secure` step, formerly the hardening guide (a separate set: the marketplace sources plus `do-script`), and what URL posture it *advertises*. A string on a mostly-boolean surface, the `platform_default_model` precedent; both booleans are resolved server-side so the browser holds no copy of which provenances count), `telemetry_sharing_enabled` / `_hard_disabled` / `_dismissed` / `_first_value` (ent#12/ent#437 — the four booleans the first-run overlay's `sharing` step gates on (ent#581; formerly the Finish-setup consent card), served here so the step decides from a document the page already awaits and never calls the admin status route on a load it will not act on; fail-safe in the hidden direction), `claude_auth_configured` (platform Anthropic key OR any subscription — `subscription_service.is_claude_auth_configured`, shared with the ent#582 first-credential check), `enterprise_features` (registered enterprise modules; empty in OSS-only or `TRINITY_OSS_ONLY=1`) (#847) |
