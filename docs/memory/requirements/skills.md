...
- **A refusal to adopt is bounded by its own class** (#2744): the terminal *"this install already has skills sources, so it is past migration"* refusal is the designed resting state of a migrated install whose legacy key lingers — not a failure — and `sync_library` re-enters it on **every** sync, unattended under the ent#236 auto-sync loop. It is therefore `priority: "low"` + `logger.info` with a **stable** `request_id` derived from the refused URL (`skills-legacy-adoption-refused-{sha256(url)[:12]}`; the family prefix `skills-legacy-adoption-` is reserved in `operator_queue_service._RESERVED_ID_PREFIXES`, so an agent cannot pre-create the id and self-suppress the alarm through the sink's on-conflict dedup), so `create_item`'s `(agent_name, request_id)` ON CONFLICT collapses every repeat to **one** row per refused URL and an operator's dismissal sticks. A *different* refused URL still raises its own item. The two genuine failure branches — the URL failed validation, or adoption threw — keep `priority: "high"` and timestamped, repeat-visible ids **by product decision** (a repeat carries information there), not because they are bounded. The URL echoed in the log line and in `context.url` is `strip_url_credentials`-scrubbed: the validation-reject branch is exactly the one an `EmbeddedCredentialError` reaches, so the raw value there may carry a PAT (Invariant #12)
...
- **Key Features**:
  ...
  - **Manifest-based prune**: on re-inject, files the *previous injection* wrote that the new package no longer carries are deleted (previous-manifest − new-manifest, capped 200/skill) — runtime artifacts and agent-authored files are structurally untouched
  - **Name conflict = refusal, never an overwrite (#2914)**: a `.claude/skills/<name>/` that EXISTS on the agent without the platform's `.trinity-skill.json` marker is the agent's own playbook, not an older copy of the library skill — the two share one flat namespace and a name match proves nothing. Injection refuses before a byte is staged (no archive, no restore, no `.gitignore` line, no untracking, not listed under CLAUDE.md Platform Skills), on every trigger (assign, manual Sync with `force=True`, start path, fleet re-inject); status `conflict` — neither `injected` nor `failed` (overall `success` stays true, so the fleet sweep does not alarm on a standing conflict every auto-sync; `skills_conflict` + `conflicts[]` carry it). The verdict is **recorded on the assignment row** (`agent_skills.delivery_status = 'conflict'`, NULL otherwise; carried across the bulk-replace PUT for retained names) so the Skills tab shows it to an operator who never saw the assign response, with the two ways out (unassign the library skill — an inline action — or rename/remove the agent's directory and sync). Cleared by the next injection where the name lands; unassigning deletes the row. A platform-managed dir (marker present, including one a pre-#2914 assignment overwrote) keeps upgrading in place; an unreadable probe keeps the pre-existing fail-open direction. **The platform never leaves a marker-less directory behind** (merge-train finding on PR #2920): the pre-#384 legacy fallback writes the marker beside `SKILL.md` (manifest = what was actually written) and a restore that drops the marker member writes it back directly (`marker_written_directly`); if the marker cannot be written the injection is reported `failed` (`marker_not_written`, legacy write rolled back) rather than a half-managed directory the next sync would refuse forever. Directories a pre-fix legacy fallback already left marker-less are indistinguishable from agent-authored ones and resolve through the conflict's second way out (rename/remove, then sync). `deliver_assigned` names it (`conflict` when every requested name collides, `partial` + `conflicts[]` on a mix); MCP passes it through and `get_agent_skills` carries `delivery_status` per row. Before #2914 the same branch overwrote the agent's copy and buried an `unmanaged_dir_overwritten` warning in the response — recoverable only while the agent's git-sync had not yet committed the overwrite
  - **Honest per-skill results**: `{success, status: injected|unchanged|fallback|failed|conflict, files_written, error?, warnings[]}` with named warning codes (`missing_binary:*`, `missing_env:*`, `packages_not_checked`, `symlink_skipped:*`, `restore_skipped:*`, `stale_delete_failed:*`, `multi_file_dropped_old_image`, `frontmatter_invalid`, …); never silent partial success
