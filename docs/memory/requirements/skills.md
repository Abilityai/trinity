...
- **Key Features**:
  ...
  - **`scope: "all" | "accessible"`** so an empty accessible set is worded honestly: a `role=user` with no agents has the assigned-to row **suppressed**, never rendered as "No agents yet" — which would assert zero holders for a skill that may be on forty agents
  ...
  - **Tabs**: `OverflowTabs` (the design-system contract's mandated primitive — `Operations.vue`'s hand-rolled strip is not the precedent), `?tab=` URL state, Back/Forward honored via a `route.query.tab` watch, legacy `#agent-templates`/`#systems`/`#skills` anchors migrated to the matching tab then cleared, `?tab=` winning over a hash. The Systems tab stays creator-gated with **both** arms of a late-role watch (`stores/auth.js` reports `user` until `/api/users/me` lands, so a creator hard-loading `?tab=systems` would otherwise land on Templates and never recover)
