...
- **Not Built**: per-source `skills_path` — the `.claude/skills/` layout is a fixed convention for every source, so a repo shaped differently syncs to zero skills. Per-source automation schedules: auto-sync (§21.7, ent#236) is one library-wide timer over every enabled source, not a cadence per source
