...
- **Create-time resolution contract (#1759)**: `local:<name>` is resolved against the curated catalog first, then the deploy-local store (`/data/deployed-templates`, #950). A well-formed but **unresolvable** id now fails with a named **400 `LOCAL_TEMPLATE_NOT_FOUND`** raised **before any side effect** (no container, no MCP key, no volume, nothing to roll back) — completing the loud-reject contract #843 opened for *unprefixed* template strings. An empty / non-mapping / unparseable `template.yaml` fails the same way with **400 `LOCAL_TEMPLATE_INVALID`**, matching the strictness the listing surface (`GET /api/templates`) already applied. The traversal barrier keeps precedence: a malformed name is still 400 `INVALID_LOCAL_TEMPLATE_NAME`. `template: null` / `""` (Blank Agent) never enter this branch and are unaffected.
