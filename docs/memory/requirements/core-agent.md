...
- **Content**: the `.gitignore` mirrors `git_service._GITIGNORE_PATTERNS` — the single source of truth the platform re-appends on every git sync. Shipping it in the template moves protection from *first sync* to *first boot*, the window a freshly created agent's compatibility report observes, and covers `local:` agents that never sync at all. Because the file is byte-identical to what the sync merge and the `#668` auto-fix would write, birth-state == post-auto-fix state == post-sync state. A template's own pre-existing exclusions are preserved in a trailing `# Template-specific` section (today: `outputs/` on the `dd-*` fleet).
