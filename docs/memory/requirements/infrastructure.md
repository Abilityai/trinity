...
- **Requirements**:
  ...
  - **PROV-009** (two steps that stack, not alternatives): a real domain (point an A record at the droplet and save it as the Public URL; on a `--provision` host Caddy then obtains an ordinary certificate for it on first request, PROV-015) **then** a Cloudflare Tunnel, so the server stops listening on the public internet. A tunnel, not a VPN (decided 2026-09-01, #2564): a VPN reaches the same posture but breaks every inbound integration — Telegram, WhatsApp, VoIP, public agent links and webhook triggers all call in. VPN stays a documented deployment mode in `docs/DEPLOYMENT.md`; it is off the card only. The tunnel needs the name, so the card presents them as successive steps, never either/or, and the tunnel step carries no button: `TUNNEL_TOKEN` lives in `.env` and the service starts under a compose profile, neither of which a container can reach.
  - **PROV-010** (two stages, each dismissible, advanced by server state): `components/onboarding/HardeningGuide.vue` renders only when the flags have loaded AND the viewer is a verified admin AND `hardening_guide_eligible` AND the current stage is not dismissed (`hardeningGuide.js::isHardeningGuideVisible`). Posture selects the **stage**, not visibility: `hardeningStage()` maps `https-domain` → `tunnel` and anything else → `address`, so a configured domain **advances** the card to the tunnel step instead of retiring it — retiring on step one meant step two was mentioned once and never again. Dismissal is localStorage **per stage** (`dismissKeyForStage()`; the address stage keeps the original `trinity_hardening_guide_dismissed` key, so existing dismissals hold), so waving away step one cannot spend step two, and a dismissal is the only thing that ends the guide. Admin is a real gate: the address stage's one action, **Add a domain**, opens Settings → General, which is `adminOnly`. On a `--provision` host that field is the whole step, because Caddy obtains the certificate itself behind the backend's `ask` gate (PROV-015). **Server state, not verified fact** — the posture reads an operator-declared address, so any https domain in `public_chat_url` advances the card whether or not DNS resolves or a certificate exists; accepted because it only moves a nudge, while PROV-003's bar decides whether the surface may exist. The copy still claims configuration, never a handshake: Trinity issues no certificate itself; the web server in front "is configured to obtain one for the name you save".
