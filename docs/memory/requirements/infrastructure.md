...
- **Requirements**:
  ...
  - **PROV-017** (Vultr is imageless — the script IS the artifact, #2282): the Vultr Marketplace listing carries no snapshot. Vultr runs `scripts/deploy/vultr-vendor-data.sh` once, as root, through cloud-init on a clean **Ubuntu 24.04** (Ubuntu because `provision_machine` installs Docker from Docker's Ubuntu apt repository), and it does what a snapshot would otherwise have baked: resolve the release, clone it, call `start.sh --provision --cloud vultr --provenance vultr-marketplace --hosted --unattended`. First boot therefore costs ~4 minutes (apt + a ~1.6 GB image pull; measured end to end on a `vc2-4c-8gb` in Frankfurt) against the DigitalOcean image's ~2 — the deliberate trade for having nothing to rebuild, re-review and re-publish per release. **The release is resolved, not carried** (`releases/latest`, which excludes the RCs `/release` cuts with `--prerelease`), because the copy that actually runs lives in Vultr's portal: a literal in this repo would be a pin nothing can enforce, and a `VERSION` test would guard the wrong copy while forcing an RC tag into it mid-cycle. A failed resolve ABORTS — a successful install of the wrong release is invisible to the customer and to us. `TRINITY_IMAGE_TAG` is **exported**, or `start.sh` persists nothing and `resolve_image_tag` silently falls back to the `latest` docker tag. **No admin is provisioned** (ent#580): `ADMIN_PASSWORD_SOURCE=browser`, so the first visitor claims the instance at `/setup` and no password ever enters Vultr's metadata service, which serves app variables to every host process for the life of the machine (ent#622 item 2). The IP comes from Vultr's plain-text value endpoint `http://169.254.169.254/v1/interfaces/0/ipv4/address` — no header, no JSON: that read is also the "am I on a cloud VM" guard and runs BEFORE any package is installed, and stock Ubuntu has curl but not jq. **Honest status without a snapshot**: the script installs its own `/etc/update-motd.d/99-trinity` before anything can fail and writes `/etc/trinity/firstboot-failed` from an EXIT trap (`set -e` exits silently), so a boot that dies still says why and prints a retry command; the clone is guarded by `[ -d /opt/trinity/.git ]` so that retry resumes. `umask 022`, never 077 — the backend runs as UID 1000 and bind-mounts `./config` read-only, and a root-only clone empties the template catalog with no error at install time. A plan under the 8 GB floor (HOST-011) warns and installs anyway, since Vultr cannot enforce a plan size and a running instance with an honest banner beats one that refuses a plan already paid for. Listing copy, the vendor-portal App Instructions and the per-release runbook: `scripts/deploy/vultr-marketplace.md`.
