...
- **Requirements**:
  ...
  - **PROV-010** (skippable and self-retiring): since ent#581 the guide is the `secure` step of the first-run overlay (`components/onboarding/FirstRunOverlay.vue` over the `firstRunSteps.js` registry; it replaced `HardeningGuide.vue`). It applies only when the flags have loaded AND the admin is on a `marketplace_install` AND the posture is not `https-domain`; a skipped step does not re-open the overlay. Skipping is localStorage (the ent#319 `firstRun.js` precedent — no new endpoint, no server row; a pre-ent#581 guide dismissal counts as a skip); retirement is server state, so a configured domain retires it with no client state at all. **Server state, not verified fact** — the posture reads an operator-declared address, so any https domain in `public_chat_url` retires the step whether or not DNS or a certificate exists. Deliberate (it is the AC's completion condition, and an admin can already skip outright), and deliberately held to a lower bar than PROV-003: that one decides whether the surface may exist, this one only decides whether a nudge is still useful. The step is also explicit that **Trinity issues no certificate** — setting the Public URL changes the name Trinity hands out; whatever terminates TLS in front of it acts on that.
...
- **Tests**: `tests/unit/test_2380_install_provenance.py`, `src/frontend/tests/unit/hardeningGuide.spec.js`, `src/frontend/tests/unit/firstRunSteps.spec.js`
