...
- **Key Features**:
  ...
  - **A shared file is for the person the turn was for (trinity-enterprise#549, ✅ 2026-09-21).** The platform decides the addressee from the turn; the agent does not choose. A Workspace chat or a room turn with Ada → Ada; a WhatsApp / Telegram / Slack conversation → that channel identity, and Ada's Files tab when she is a verified speaker in a one-to-one chat (never in a group, where the verified email is the unlocker's, not the speaker's); a schedule, operator chat, MCP call, loop or agent-to-agent call → the owner only. The Workspace Files tab lists what is addressed to the viewer (the agent's owner also reads the owner-only files); the owner's Sharing panel lists everything and says who each file is for — to the owner in the UI; a key-authenticated caller of that list (any MCP scope, present or future) sees the rows but neither *who* nor *whether* they were addressed — `addressed_to`, `addressed_to_channel` and `audience_source` are all withheld (#2955). One normaliser (`utils/addressee.py`) serves every writer, the reader and both `audience_email` request validators. One override: `audience_email` names a different person on the agent's roster, and an off-roster address is refused by name (`AUDIENCE_NOT_ON_ROSTER`), never silently widened. Existing shares become owner-only — their recipient is unknowable, and every share expires within seven days. Deciding *which turn a share came from* needs positive evidence (the agent's own key and an `execution_id` of its own); without it the file is the owner's, and the tool result says so (`visible_to_requester: false` + `visibility_note`). Unchanged: the `?sig=` link is a bearer credential — the audience governs the listing, not the download. Out of scope: a user sharing a file onward (ent#633), per-file ACLs (ent#484 / ent#489).
- **Database**: `agent_shared_files` table (+ `addressed_to_email` / `addressed_to_channel` / `audience_source`, ent#549 — nullable, no backfill, dual-track) + `agent_ownership.file_sharing_enabled` column (FK `ON DELETE CASCADE ON UPDATE CASCADE`, though enforcement is via the manual-cascade pattern used platform-wide)
