...
- **Background tasks (ent#551, Workspace path)** — a long task runs while the conversation continues:
  ...
  - **Once, not three times.** The acceptance asks for a spoken line only if the model has not already said what it started (the first cut produced "let me check… / I've started… / it's done" back to back). The spoken request is flushed to the thread before the task's rows are written, so the rows read in the order things happened. The notice names the task and its request so the model can say which ask it answers; the result rides in clipped to `_TASK_RESULT_MAX` chars (the full reply is in the chat).
