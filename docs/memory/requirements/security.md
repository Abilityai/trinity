...
### 20.5c Enforcing-Window Reporting on the Headroom Payload (Abilityai/lilu#74)
- **Status**: ✅ Implemented (2026-09-14, lab branch `lilu`)
- **GitHub Issue**: Abilityai/lilu#74 (P1, `theme-reliability`, `gap`); related Abilityai/lilu#62 (staleness — a different failure of the same endpoint), Abilityai/lilu#81 (this endpoint's authorization, deliberately untouched here)
- **Extends**: 20.5a (#471)
- **Description**: `GET /api/subscriptions/{id}/usage` and the runtime that actually rejects turns were describing **different windows with different resets**, so the headroom read could say `allowed` at the exact moment the platform was refusing every turn. Root cause: header parsing read exactly two hardcoded prefixes (`5h`, `7d`), so any other window family the provider reports was parsed into nothing and could not appear anywhere. Observed 2026-09-09 on `lilu-max`: `allowed`, `seven_day 16%`, `resets_at 2026-09-15T23:00:00Z`, `snapshot_age_seconds 11` — fresh, so not the 20.5b/#62 staleness class — while the enforced limit was a weekly one resetting `2026-09-13T02:00:00Z`. Five executions died in 2–22 s with `response: null`, including an operator's Telegram question and both morning-briefing attempts, and a dashboard read of `rate_limited_now: false` produced a confidently wrong diagnosis.
- **What is reported**: `headroom.windows` — **every** `anthropic-ratelimit-unified-<window>-<field>` family present, canonically keyed — and `headroom.enforcing`, which names the binding limit, its `basis` (a **blocking window** first, since that is an observation rather than an inference and outranks the provider's `representative-claim`; then the claim; then the fullest window), and **that limit's own** `resets_at`, `utilization_pct`, `remaining_pct` and `status`.
- **Honest-gaps rule (inherited from 20.5b)**: when the provider exposes no enforcing figure the field is **absent**, never a fabricated `0` — a consumer must be able to tell "no enforcing figure" from "plenty of room". Selection is deterministic, so a dispatch gate reading it cannot see the named limit flap between equally-full windows on consecutive polls.
- **Additive contract**: no existing field removed or repurposed; existing consumers (`/experiment` Step 0b, `/update-dashboard`, the Settings panel, the Dashboard pressure tile) keep reading exactly what they read today. Both display predicates now iterate the complete window set, which is a no-op for any snapshot carrying only the two known windows — i.e. every snapshot on every install today.
- **Out of scope, by the issue's own fence**: the enforcing **runtime** (this reports it, it does not change it), #62's staleness, and this endpoint's **authorization**, which is byte-for-byte unchanged.
