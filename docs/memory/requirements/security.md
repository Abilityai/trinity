...
- **Residuals (registered)**: `closed_by_filer` rows still count toward the depth cap until cancelled (trinity-enterprise#619); a response undelivered because the entry changed *after* the answer has no re-answer path (ent#619/#611); a re-ask with a reused id after `acknowledged` is swallowed by #1631's design (ent#619).
