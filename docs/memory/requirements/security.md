...
### 26.8 Sync Honesty — divergence, delivery, aging, audit (OPS-001-HONEST)
- **Status**: ✅ Implemented (2026-09-23, #2915)
- **Requirement ID**: OPS-001-HONEST
- **Priority**: P1 (Trinity 1.0 release gate — an Inbox built on a silently stale card shows a wrong "nothing needs you")
- **Description**: The file contract (`~/.trinity/operator-queue.json` ⇄ `operator_queue`) stays as it is — ingestion is create-only and the platform never applies an agent's rewrite in place (an approval is frozen to the exact action the human read; corrections are trinity-enterprise#619, the native request object is trinity-enterprise#611). What changes is that every way the two sides can disagree is **detected, recorded on the row, rendered, and audited** instead of presenting as "fine".
  - **Sync state (per row, written only by the leader-locked poller, on change):** `sync_state ∈ {confirmed, changed, closed_by_filer, missing, stale_id, unconfirmed}` + `sync_detail` (a closed vocabulary: the changed field names; the agent's folded status for `closed_by_filer`; `entry_missing` / `file_missing`; the terminal status for `stale_id`; `timeout` / `unreachable` / `http_<code>` / `invalid_json` / `oversize_file` / `agent_not_running` for `unconfirmed`) + `sync_updated_at` (transition time) + `last_confirmed_at` (refreshed by ONE batched UPDATE per agent per cycle for rows older than 60 s — a minute cadence, never per row per cycle). Divergence is decided by a **content fingerprint** over the clamped `title` / `question` / canonical `options` / normalised `expires_at`: the row *is* the ingest snapshot, so no extra column is stored and pre-migration rows compare on first sight. A pending file entry whose id matches a **terminal** row is `stale_id` on that row — never a phantom admit (the on-conflict create returns the surviving uuid silently). Read failures enter `unconfirmed` only after **3 consecutive** failed cycles (the `sync_health` precedent) and leave it on the first good read; an unparseable or oversize file is `unconfirmed`, never `missing`, and **no write-back runs that cycle** (the previous path overwrote the agent's file with reconstructed responses only). Agents that are not running are swept to `unconfirmed:agent_not_running` from the tri-state `agent_container_states()` — `None` (Docker unreadable) sweeps nothing — and expiry runs before the "no running agents" return.
  - **Delivery state (per row):** `delivery_state ∈ {delivered, undelivered, not_applicable}` + `delivery_detail` (`write_failed` / `http_<code>` / `unreachable` / `timeout` / `conflict` / `entry_missing` / `entry_changed` / `closed_by_filer` / `platform_minted`) + `delivery_updated_at`. The write-back delivers a response **only into a matching, still-pending entry** — an entry the agent rewrote or closed gets `undelivered` with the reason, never the answer to a different question. Terminal (cancelled/expired) flips are fetched by delivery state, not by a 168 h `created_at` window, so a flip is attempted until it lands or is recorded undeliverable. Platform-minted rows are `not_applicable`. Before every write the poller **re-reads** the file, merges by id, and writes with `if_match=<sha256 of what it read>`; the agent server's `PUT /api/files` writes atomically (tmp + `os.replace`) and answers **412** on a mismatch, so a concurrent agent write is refused and retried instead of clobbered (an older agent server ignores the parameter and degrades to the narrowed window).
  - **Refusal on a diverged item:** `POST /api/operator-queue/{id}/respond` and the portal answer return **409 `item_diverged`** while `sync_state ∈ {changed, closed_by_filer}` unless the body carries `acknowledge_divergence: true`; the UI shows the divergence and resends with the override on the operator's second click.
  - **Aging:** `operator_queue_aging_hours` (ops setting, default **24**, `0` disables, validated 0–8760, readable/writable through the admin-gated `GET/PUT /api/settings/ops/config`, edited from Settings → Operations). Computed at read time — every list/get response carries `aging: bool` + `aged_since` per item; the same predicate decides the **receipt**: the poller writes `platform.aging_since` into the agent's file entry once, at the crossing, riding the write-back (never a standalone write at ingest). The prompt's Operator Communication section teaches the field.
  - **Escalation = a visible count:** the list response carries `undelivered_count` and `closed_by_filer_count`; the Operations header renders both and offers one-click cancel of `closed_by_filer` rows through the existing bulk-cancel. No new queue item is minted about queue items.
  - **Audit** (`platform_audit_service`, `event_type=operator_queue`, `source=system`, target = the row uuid, details = ids and enums only, never agent text): `ingested` · `diverged` (→ changed / closed_by_filer / missing / stale_id) · `reconciled` (diverged → confirmed) · `written_back` · `undeliverable`. The `confirmed ↔ unconfirmed` flap is **never** audited (a fleet restart writes zero rows); the edge is the UPDATE's rowcount under `col IS NULL OR col != :v`, so two overlapping leaders cannot double-write.
  - **Surfaces:** `QueueCard` / `ResolvedCard` (Operations), `/m` (MobileAdmin), and `PortalAsks` render ONE shared rule (`utils/operatorQueue.js::queueSyncBadge`); the portal projection exposes a coarse `sync ∈ {confirmed, changed, closed, unconfirmed}` + `aging` only — no reason, no timestamp. MCP `list_operator_queue` passes the fields through.
- **Files**: `src/backend/services/operator_queue_service.py`, `src/backend/db/operator_queue.py`, `src/backend/routers/operator_queue.py`, `src/backend/client_portal/asks/{models,service}.py`, `src/backend/services/agent_client/client.py`, `docker/base-image/agent_server/routers/files.py`, `src/backend/config.py`, `src/frontend/src/utils/operatorQueue.js` + the four callers, migrations `operator_queue_sync_state` (SQLite) / `0073_operator_queue_sync_state` (Alembic)
- **Tests**: `tests/unit/test_2915_operator_queue_sync_honesty.py`, `tests/unit/test_2915_agent_server_files_if_match.py`, `src/frontend/tests/unit/operatorQueueSyncBadge.spec.js`, `src/frontend/tests/unit/queueCardSyncBadge.spec.js`
- **Residuals (registered)**: `closed_by_filer` rows still count toward the depth cap until cancelled (trinity-enterprise#619); a response undelivered because the entry changed *after* the answer has no re-answer path (ent#619/#611 — narrowed by OPS-001-ENDINGS: the agent reads the answer back by its own `request_id` over MCP, so a refused file write-back no longer leaves it without the decision); a re-ask with a reused id after `acknowledged` is swallowed by #1631's design (ent#619).
...
- **Description**: An ask ends in exactly one of three ways — answered, cancelled or expired. Every ending is recorded on the row, audited once, delivered to the agent that raised it, and shown with who and when on every surface that lists the ask.
  - **Endings ledger (per row; nullable, no backfill):** `disposition ∈ {answered, cancelled, expired}`, `disposed_at`, `disposed_by ∈ {person, timeout}` (an enum of two), `disposed_by_email` (NULL for timeout), `disposition_reason` (the operator's optional cancel reason, ≤ 500 chars) and `batch_id` (one uuid per bulk-cancel sweep). Each is written in the SAME compare-and-set UPDATE that flips `status`, so a writer that loses the race writes nothing. Rows that ended before the ledger keep a NULL `disposition` and read from `status`; no surface presents `created_at` as an ending time. The same migration adds the columns agent-raised asks need (`raised_by`, `channel`, `to_role`, `resolved_to`, `proposal`, `supersedes_expired`) — written by PR B, never read from an agent's file entry.
  - **One transition sink (`services/ask_service.py`):** the operator answer, the client-portal answer, single and bulk cancel, and expiry all end an ask through one service: compare-and-set writer → one audit row per transition → one thin WebSocket trigger → the registered ending observers. Observers only ever receive the rows whose compare-and-set this call won. The ent#329 wake is the default observer; other features register their own and nothing feature-specific branches inside the sink.
  - **Only a person ends an ask:** respond, cancel and bulk-cancel accept an interactive session or a user-scoped MCP key, and the Workspace answer applies the same rule to a signed-in platform principal (a system-scoped key keeps its Workspace read breadth but cannot answer). Agent-, system- and every other scoped key gets **403 `person_required`** (an allowlist, so a scope added later is refused by default). Before this an agent-scoped key resolved to its owner and could answer or cancel any ask the owner could reach — its own included — recorded as the owner.
  - **No answer after the deadline:** the respond compare-and-set also requires `expires_at IS NULL OR expires_at > now`, so an answer that arrives after the deadline is **409 `expired`** even before the poller sweeps the row. `expires_at` is stored as ISO-8601 UTC with a `Z` suffix at file ingest (Invariant #16).
  - **Cancel is audited and raced like respond:** single cancel takes an optional `reason` (≤ 500 chars) and answers **409** when the row already left `pending`; bulk-cancel returns its sweep's `batch_id` and audits the ids it actually cancelled, not the ids requested.
...
- **Description**: An agent asks a person for a decision through ONE platform call — validated at the call, stored, announced and answered with a receipt — instead of appending to its queue file and waiting for the poller.
  ...
  - **Receipt:** `status`, `id`, `request_id`, `channel`, `type`, `to_role`, `resolved`, `ask_status`, `disposition`, `disposed_at`, `expires_at`, `wakes_on_ending` (the owner's wake opt-in), `supersedes_expired` (the predecessor's `request_id`), `differs` on a replay — the role, never a person's email. The reads keep the rule: on a native row the operator queue's get and list routes withhold `addressed_to_email` (the resolved person, like `resolved_to`) from every non-person principal.
  ...
  - **Audit** (`event_type=operator_queue`, ids and enums only): `raised` (`source` api or system), attributed to the agent — never to the owner its key resolves to — with the presented key and the owner's email as the join back to the human; plus one thin `operator_queue_new` trigger.
