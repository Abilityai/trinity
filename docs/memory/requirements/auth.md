...
- **`slack.py` deferred (10 sites).** Overlaps the channel-adapter area — coordinated follow-up PR after sign-off. Each deferred inline-auth line carries a `# noqa: inv8` marker so the static guard still trips on **new** inline auth added there.
- **Static guard**: `tests/unit/test_1310_auth_wiring.py` — a precise AST matcher forbids `db.can_user_access_agent(` / `db.can_user_share_agent(` Call-nodes whose negation guards a `raise HTTPException`, and inline `role != "admin"` deny-`If`s, in `routers/` (per-function allowlist; `# noqa: inv8` line-exemption for the deferred file). Filter/capability/allow-branch sites (assignments, `role == "admin"` selection, WS-`close(4003)` dict handlers) are **not** flagged.
