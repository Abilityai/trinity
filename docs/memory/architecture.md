...
| `connector` | Connector-only tool set (`list_playbooks` / `run_playbook` / `ask`); operator tools hidden | **Fenced to two routes** — its bound agent's `/chat` + `/connector/playbooks`; every other path 403s (ent#46 → #118) |
