...
| **Canary Watcher** | `canary_service.py` | 5-min invariant harness cycle. **Cross-worker leader lease (#1881):** the loop runs in every uvicorn worker but only the holder of the Redis `canary:leader` lease (SET NX, TTL `max(3×interval, 900s)` floor, own-lease-only refresh, release on `stop()`; fail-open to leader when Redis is down) runs a cycle — mirror `monitoring:leader` #1464 / `opqueue:leader` #1632 — so `--workers 2` no longer runs R-01's per-agent `docker exec` sweep twice per 5 min, double-persists `canary_violations`, or gives every cross-cycle marker two writers; leadership fails over when the holder dies. `run_cycle()` (the on-demand `POST /api/canary/run-cycle`) is deliberately **not** gated. See [Canary Harness](#canary-invariant-harness-canary-001-411) |
