...
- **Fleet-restart lock** (#1860): `ops:fleet_restart` — SETNX + 900s TTL single-flight guard for `POST /api/ops/fleet/restart` (409 on contention); the live loop refreshes its own lease per agent, release is compare-and-delete. Not `agent:*`-named (fleet-scoped, #1560 registry doesn't apply); fail-open when Redis is down.
