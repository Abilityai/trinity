...
8. **Resume-failure fallback** (Phase 2.2): if execute_task returned failed with `"no conversation found"` AND we had a cached UUID, clear the cache, `mark_resume_failure`, retry **once** with `resume_session_id=None`. Log structured warning `event=session_resume_fallback`. Anthropic #39667 (cleanupPeriodDays) and #53417 (CLI upgrade) both produce this signal. **Hazard — this fallback is carried by third-party free text, not a typed code (#1849).** `_is_resume_not_found` substring-matches Claude Code's own error string after it has survived four transforms: the agent-side `[ede_diagnostic]` filter + join (`stream_parser._split_claude_errors`), `sanitize_text`, and `[:300]` truncation in `headless_executor`, before crossing the agent→backend HTTP boundary as a 502 detail. It has broken **twice** — #1673 (the text was dropped entirely) and #1849 (the CLI's internal diagnostic header displaced it at `errors[0]`) — both times with green CI and no signal, because nothing pins the contract end to end. Any change to the parser, the sanitizer, or the truncation budget must be validated against `tests/unit/test_1849_ede_diagnostic_filtered.py::test_resume_fallback_fires_through_the_502_detail`, which drives the whole chain into the real predicate. `ExecutionMetadata.error_code` exists and is the intended replacement (typed code preferred, substring kept as an old-image fallback) — tracked as a follow-up.
