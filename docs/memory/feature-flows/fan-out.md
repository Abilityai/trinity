...
- **#2524, dual-track (Invariant #9)**: `_migrate_execution_fan_out_task_id()` in `db/migrations.py` **and** Alembic `0059_execution_fan_out_task_id` (chained after `0058_portal_file_dismissals`). Adds `fan_out_task_id TEXT` plus a composite `idx_executions_fan_out_status ON schedule_executions(fan_out_id, status)` — the join COUNTs non-terminal rows for one batch on every fan-out terminal, which the single-column index cannot serve without reading the whole batch.
