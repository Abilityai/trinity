...
## Recent Changes
- **#2806 (chain-depth guard)**: the router calls `dispatch_admission_service.enforce_inter_agent_depth()` after the target check and before `idempotency_service.begin`. An agent-principal batch past `inter_agent_max_chain_depth` → 403 `inter_agent_depth_exceeded` (nothing dispatched, no key burned). An admitted depth is captured at request time and passed as `FanOutService.execute(chain_depth=)` → `_dispatch_all` → every subtask's `create_task_execution(chain_depth=)`, so a subtask granted a slot later still carries it. See `agent-to-agent-collaboration.md` → Chain-Depth Guard.
...
1a. Chain-depth guard (#2806): `enforce_inter_agent_depth(current_user, target=name, ...)` → 403 `inter_agent_depth_exceeded` or the depth to stamp (None for a non-agent principal)
