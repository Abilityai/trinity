...
> **Status**: Implemented
> **Created**: 2025-12-22
> **Updated**: 2026-08-14 (#2127: the #970 early-finalize is gated on the CLI's background-task ledger, so a fan-out turn is no longer killed mid-wait and recorded as success; prior 2026-05-18 #873: pipe-drop handler ported to refactored `headless_executor.py` after #122 extraction)
> **Verified**: 2026-02-05
...
| `"execution_error"` | **502**, or **200 + recovered response** when the transcript shows an in-turn `stop_reason=end_turn` (#1870) | Dedicated branch in `headless_executor._finalize_headless_result` (#1673); recovery via `_try_recover_completed_turn` (#1870) |
