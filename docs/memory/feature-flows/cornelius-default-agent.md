...
4. **Cross-worker lock** — Redis `SETNX cornelius:provision` via the shared ownership-checked `SingleFlightLock` (#1920: unique per-acquire token + compare-and-delete release, so a slow pass whose TTL lapsed can't delete a *successor's* lock — the pre-#1920 constant-"1" + unconditional-delete bug it shared with system_seed), fail-open so `--workers 2` boots can't both provision. If the lock isn't acquired, the other worker owns it.
