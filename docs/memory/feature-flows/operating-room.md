...
| `pending` entry re-using a **terminal** row's id (the on-conflict create would return the old uuid silently — a phantom admit + "new" broadcast every cycle) — but only one this cycle's write-back will NOT flip (`awaits_terminal_flip` false: the flip already landed, the entry was missing, or the row is `acknowledged`). The reconcile runs before the write-back, so the ORIGINAL entry of a row the platform just cancelled or expired is still `pending` here and is not a re-use (#3024) | `stale_id` (on the terminal row) | the terminal status |
| the entry reads the terminal row's own status while the row reads `stale_id` (a flag set before #3024) | `confirmed` (audited `reconciled`) | — |
