...
| present, content differs (`title` / `question` / `options` / `expires_at`, compared through a pure fingerprint: the same caps and `create_item` defaults the row was stored with, canonical options JSON, normalised `expires_at`) | `changed` | the changed field names |
...
| open row absent from the file / file absent | `missing` | `entry_missing` / `file_missing` |
...
| read failed 3 cycles in a row (hysteresis; the 5 s read times out on a busy container), file unparseable / oversize, agent not running, Docker unreadable | `unconfirmed` | `timeout` / `unreachable` / `http_<code>` / `invalid_json` / `oversize_file` / `agent_not_running` |

Writers are edge-triggered (`WHERE sync_state IS NULL OR != :v …`; the rowcount **is** the transition, never read-then-write, so two overlapping leaders cannot double-record); `last_confirmed_at` is refreshed by ONE batched UPDATE per agent per cycle for confirmed rows older than 60 s. An unparseable or oversize file marks `unconfirmed` and **runs no write-back that cycle** — the previous path treated it as an empty request list and overwrote the agent's file with the reconstructed responses alone. `_poll_cycle` now reads the tri-state `docker_service.agent_container_states()` (`None` = Docker unreadable ⇒ nothing swept, nothing synced — the #2196 class), runs expiry **before** the "no running agents" return (an all-stopped fleet never expired anything), sweeps open rows of non-running agents to `unconfirmed:agent_not_running` (explicit empty-set branch, never `notin_([])`), and broadcasts ONE thin `operator_queue_sync` per cycle when any row changed.

**Write-back (`_write_responses_to_agent`).** Re-reads the file immediately before writing, merges by id, and writes with `if_match=<sha256 of the UTF-8 text it read>`; the agent server's `PUT /api/files` writes atomically (tmp + `os.replace`) and answers **412** on a mismatch, so an entry the agent appended in between is refused-and-retried, never clobbered (an older agent server ignores the parameter and degrades to the narrowed window). A response is delivered **only into an entry that is still `pending` and whose content still matches the row** — an answer to a rewritten question is `undelivered:entry_changed`, to a closed one `undelivered:closed_by_filer`. Terminal (cancelled/expired) flips are fetched by delivery state (`db.get_operator_queue_terminal_for_agent`: `delivery_state IS NULL OR 'undelivered'`, capped 200) instead of a 168 h `created_at` window, so a flip is attempted until it lands or is recorded (`undelivered:entry_missing`). Platform-minted rows are `not_applicable:platform_minted` (whether still `responded` or already terminal — and they take NO part in delivery even when a pre-ent#499 file still carries them as `responded` entries; matching those flip-flopped delivered ↔ not_applicable every cycle on the first live run). **Upgrade-time one-off:** the first cycles after this lands record every pre-existing cancelled/expired row whose entry is already gone as `undelivered:entry_missing`, once each (edge-triggered; ≤200 per agent per cycle), so the header count is honest about history rather than silent about it. Every write outcome lands as `delivery_state` (`delivered` / `undelivered:<conflict|http_<code>|unreachable|timeout>`). The aging receipt (`platform.aging_since`, written once at the crossing, taught in the prompt's Operator Communication section) rides the same write — never a standalone write at ingest, which would race the agent's own turn.
...
**Residuals (registered in the debt inbox, owned by trinity-enterprise#619/#611):** `closed_by_filer` rows still count toward the #1632 depth cap until cancelled; a response undelivered because the entry changed *after* the answer has no re-answer path; a re-ask with a reused id after `acknowledged` is swallowed by #1631's design.

**Tests.** `tests/unit/test_2915_operator_queue_sync_honesty.py` (the seven divergence fixtures, hysteresis, no write on invalid JSON, tri-state sweep, one broadcast per cycle, write-back gating and `if_match`, the pre-write re-read, aging receipt once, fingerprint stability, and the accessors on a real migrated SQLite); `tests/unit/test_2915_agent_server_files_if_match.py`; `src/frontend/tests/unit/operatorQueueSyncBadge.spec.js` (rule + store 409 flow), `queueCardSyncBadge.spec.js` (mounted), `operatorQueueAgingSetting.spec.js` (mounted).
