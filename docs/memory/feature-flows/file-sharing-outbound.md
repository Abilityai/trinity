...
| 2026-09-23 | #2955 — `audience_source` is withheld from key-authenticated callers of the owner's list with the two address fields; the addressee normaliser moved to the leaf `utils/addressee.py` (re-exported by `turn_audience`) and both `audience_email` validators now call it, so the Share 400 `INVALID_AUDIENCE` is unreachable through the route; the owner's panel derives each row's "For" once. |
...
`normalize_addressee_email()` is the ONE spelling of an addressee for every writer, for the reader and for both `audience_email` request validators (strip + lower-case; anything not email-shaped is `None`): rooms stamp a raw `current_user.email`, while portal identities are lower-cased at login. It lives in the pure leaf `utils/addressee.py` (imports only `typing`, so `models.py` can use it without importing a service) and `turn_audience` re-exports it under the same name. `ShareFileMcpRequest` and `ReportCreate` both call it through `models._validate_audience_email` — blank is absent, otherwise the resolver's value, or a 422 `audience_email must be an email address` exactly where the resolver answers `None` (#2955; the validators previously checked only `@` and a space, so an empty local part, an empty domain or an interior tab passed them).
...
- **Residual:** a session that carries ANOTHER conversation's history can cite a real id of it — an operator who `--resume`s a client's chat in a terminal, and also an operator chat, whose `--continue` resumes the most recent session in the agent's home. If that client has a turn running at that moment and the model prefers the stale id to its own Execution Context, the file is addressed to the client. A platform-injected id (#2392) closes it.
...
| Share: `audience_email` is not email-shaped after normalisation | 400 | `INVALID_AUDIENCE: audience_email must be an email address` (since #2955 every value the resolver rejects — no `@`, an empty local part, an empty domain, interior whitespace of any kind — is a 422 from the `ShareFileMcpRequest` validator before the service sees it, so this 400 is unreachable through the route and is kept for direct callers of `create_share`; the empty string is "absent") |
