...
## Overview
Per-subscription rolling token and cost usage across two time windows (5h and 7d), plus — since #471 — failure-event counters and **live provider headroom** (actual 5h/7d utilization % + reset times), and — since Abilityai/lilu#74 — **every** window family the provider reports plus `enforcing`, the one that actually binds. **Dedup rule (#471, verified live):** a modern turn writes BOTH a `schedule_executions` row and (for `/chat` + persisted `/task`) a cost-bearing `chat_messages` row, so `schedule_executions` is the SOLE source for cost / context-estimate / turn count; `chat_messages` contributes only `output_tokens` (which executions don't carry). Summing both — the pre-#471 behavior — counted every chat turn's cost twice.
...
## Abilityai/lilu#74 — Which limit is actually enforcing

**The defect the section exists for.** `parse_unified_headers` read exactly two hardcoded prefixes, `5h` and `7d`. A window family the provider reports but this code has never heard of was parsed into **nothing** — not "unknown", *nothing* — so it was invisible to the payload, to both display predicates, to `subscription_headroom_history`, and to the Dashboard tile. On **2026-09-09** that let `/usage` report `status: allowed`, `seven_day: 16%`, `resets_at: 2026-09-15T23:00:00Z` with `snapshot_age_seconds: 11` (so **not** #62's staleness — the snapshot was fresh and simply described a different window) while the runtime refused every turn on a weekly limit resetting **2026-09-13T02:00:00Z**. Five `lilu` executions died in 2–22 s with `response: null`, including an operator's Telegram question and both attempts at the morning briefing, and the 12:05Z dashboard read `rate_limited_now: false` and concluded "it was not the subscription" — the diagnosis a wrong number buys you.

Two changes, both additive:

- **Discovery is generic.** Every `anthropic-ratelimit-unified-<window>-<field>` family present lands in `headroom.windows`, keyed by `canonical_window_name` (`5h`→`five_hour`, `7d`→`seven_day`, `7d-opus`→`seven_day_opus` — only the head token is translated, so a qualified window stays recognisable as the weekly family it belongs to **and** stays string-equal to what `representative-claim` emits). `five_hour`/`seven_day` are unchanged and still populated by name. Non-window families that share the `<segment>-<field>` shape (`overage-*`, `representative-*`, `fallback-*`) are excluded by name; an *unnamed* family carrying only a `-status` is excluded structurally, while the two **known** windows keep theirs — a 429 reports a window status with no figure, and dropping it would delete the very signal `_headroom_indicates_limited` reads off that response.
- **`headroom.enforcing` names the binding limit**, with the evidence: a **blocking window** first (`basis: blocking_status` — the only arm that is a direct observation rather than an inference, so it outranks the provider's own nomination), then `representative-claim` when it names a window we received, then the fullest window by utilization. It carries **that** limit's `resets_at`, `utilization_pct`, `remaining_pct` and `status`. It is **absent**, never a fabricated `0`, when there is nothing to choose from — a consumer must be able to tell "no enforcing figure" from "plenty of room" (the ent#100 rule). Selection is deterministic (name as the tiebreak), so a gate reading it cannot see the named limit flap between two equally-full windows on consecutive polls.

Both display predicates now iterate **`reported_windows`** — the complete set — instead of the literal `(five_hour, seven_day)` pair. Without that the reported bug simply survives for the new family: a window actively refusing still could not move a badge. For a snapshot carrying only the two known windows (every snapshot on every install today) the iteration is identical, pinned by test.

**Back-compat that is load-bearing.** Snapshots live in Redis for **7 days**, so for a week after this ships most reads are of dicts written by the old parser, which have no `windows` key. `_to_model` reconstructs the map from the fixed pair and re-derives `enforcing` for those. Without it every cached snapshot would read as "no windows reported", `_headroom_indicates_healthy` returns `False` on that, and a stale `LIMIT` badge would re-pin on every subscription until the cache turned over — #447, reintroduced by a fix for a different bug.

**Deliberately out of scope** (issue's own fence): the enforcing **runtime** is untouched — this reports it, it does not change it; #62's staleness is a separate failure of the same endpoint; and the endpoint's **authorization** is byte-for-byte unchanged, since #81 owns that. `insert_headroom_history` reads named keys, so the extra snapshot key rides along without a schema change. Tests: `tests/unit/test_lilu74_enforcing_window.py`.
