...
- `POST /v1/messages` under the same token returns the full `anthropic-ratelimit-unified-*` header set: `{5h,7d}-utilization` (a fraction of the cap, NOT clamped — an overage plan reads past 1.0 and it is honoured as >100%, matching the provider's own client; #2419), `-reset` (unix), per-window `-status`, `representative-claim`, overage status.
...
- Snapshot in Redis `subscription:headroom:{id}` (7d TTL, best-effort DEL on subscription delete); probe single-flighted (`SingleFlightLock` #1920). A probe 429 updates the snapshot's `status` only — never `subscription_rate_limit_events` (platform-caused, not agent work). 401/403 → `status: invalid_token`. Missing header family on a 200 → throttled WARNING (provider drift is loud) + degrade to observed. A window reading past 100% logs the RAW header strings at INFO (`[#2419]`) — the capture path for the first real overage sample, since every local plan carries `overage_status = rejected` and blocks at 100%.
