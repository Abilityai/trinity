...
- **`components/CompatibilityPanel.vue`** (rendered inside `OverviewPanel.vue`, section 2b) reuses the Overview "needs attention" idiom: a compact summary line (tone = danger/success/gray) that expands to the full checklist grouped by category. Per-check **Fix** button for auto-fixable failures; **Re-run analysis** forces fresh AI; AI staleness stamp (`ai_ran_at`). Explanations render via `utils/markdown.js` `renderMarkdown` (DOMPurify).
- **`stores/agents.js`**: `getCompatibility(name, {includeAi})` + `fixCompatibilityIssue(name, checkId)`.
- **The headline counts must-fix only, and skipped AI checks collapse (#2899).** The summary line counted `hard + soft` in one warning-toned number, so an agent whose findings were all advisory announced "5 compatibility issues" in a warning banner on its landing tab — the first screen after a fresh install. It now reads `N must-fix compatibility issues` (danger) when `hard_count > 0`, `No must-fix issues · N recommendations` (neutral) when only soft findings remain, and `Compatible — all checks passing` (success) only at zero. **Neutral, not success**, on the advisory arm: the API still calls that state `issues` (`overall_status` is unchanged, and `ImportValidationStep.vue` + the MCP tool keep reading it that way) and SOFT covers security findings (S-006/S-008) nobody should read as "all clear". The dead `infoCount` computed is gone. In the expanded list, AI checks that could not run (`no_api_key`, `ai_not_run`, `ai_no_result`) leave the per-category rows and become ONE line per reason above it — on a keyless install that was 30 rows out of 89, burying the real findings. A skipped AI check is the ABSENCE of a finding, and `_counts` already ignored it; only the rendering treated it as content. Pinned by `src/frontend/tests/unit/compatibilityPanel.spec.js` (mounted, not grepped).
...
- **The resource checks agree with the create path (#2899).** `T-004`/`T-005` failed an ABSENT `resources` block, which is how all three bundled starters — and every `local:default` agent — greeted their owner with 2 HARD findings on a fresh install. Absence is the SUPPORTED shape: the agent inherits the admin's fleet-wide default (RES-001), and a template-level block overrides both that default and whatever a manifest or API caller asked for (`crud._resolve_local_template`). Absent (or `null`, or an empty value) now PASSES with a message naming the default; a DECLARED value is validated by the create path's own `normalize_cpu`/`normalize_memory` rather than a second copy of the value set, so the check accepts exactly what creation accepts (`4G`, `cpu: 2` unquoted) and rejects exactly what it rejects (`0.5`, `512Mi`, and `512m` — which T-005's old regex passed while creation 400s it). A non-mapping `resources:` fails T-004 and skips T-005, one finding per malformed block. Consequence: `tests/unit/test_1908_bundled_template_gitignore.py::_KNOWN_FAILING_CHECKS` is now EMPTY, so every visible bundled template is held to every HARD static check with nothing exempt — the guard half of #1908 that the two waived ids were parked on. The same value sets are enforced on the admin default itself: the generic `PUT /api/settings/{key}` catch-all refuses `agent_default_cpu`/`agent_default_memory`, since an unvalidated default 400s every creation that relies on it.
- **`X-004` exempts the platform-injected `trinity` server (#2899).** The agent's own boot writes that entry into `.mcp.json` (`agent_server/services/trinity_mcp.py`), so declaring it in `template.yaml` and omitting it from `.mcp.json.template` is correct, not drift. Exempted in that direction only — a template shipping its own `trinity` block is still reported, because the injection overwrites it when Trinity MCP is configured and leaves it standing when it is not. The name lives in `template_service.PLATFORM_INJECTED_MCP_SERVERS`, beside the env-var twin `_PLATFORM_INJECTED_EXACT` that K-001 already uses; a static mirror, since the backend cannot import the agent image (Invariant #5).
