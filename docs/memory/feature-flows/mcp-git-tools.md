...
- **`src/mcp-server/src/tools/git.ts`** (new) — `createGitTools(client, requireApiKey)` returns the six tools.
  - **`run()`** shared runner: gates the target agent (`checkAgentAccess`), mints `requestId = randomUUID()`, **stamps it on `context.requestId` BEFORE the await** (so the `mcp_operation` row carries it even when the call throws), runs the proxied call, and normalizes errors. A 409 `ApiError` is returned as a structured conflict object (`{error:"conflict", status:409, conflict_type, conflict_class, detail, hint}`) with a hint back to `chat_with_agent`; everything else → `{error}`. Exception (ent#123): `conflict_type === "no_write_credentials"` (a tokenless/anonymous-clone agent hit a push path) gets a different hint — a chat turn cannot fix missing credentials, so it points at the in-place retrofit instead of `chat_with_agent` — **ent#109**: *bind this agent to a repo you own from its Git tab, or add a GitHub token* (it used to teach the fork-a-new-agent-and-import workaround). Kept in sync with `git_service.NO_WRITE_CREDENTIALS_MESSAGE` (Invariant #13) by the source guard in `tests/unit/test_ent109_no_write_credentials_message.py`; see [agent-repo-binding.md](agent-repo-binding.md).
