...
- **Recently ended** (trinity-enterprise#611): under the Queue, the ≤ 5 items that ended most recently — answered, cancelled or expired — with who and when (`utils/operatorQueue.js::recentlyEnded` / `queueEnding`), from the same fetch the Queue already makes (it used to drop them, so an ask cancelled or expired elsewhere simply vanished from the phone). Shown whatever the pending list's state, empty included.
