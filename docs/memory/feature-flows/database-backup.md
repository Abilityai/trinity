...
**Status** (durable `system_settings`, ent#236 lesson): `db_backup_last_status` (`ok|failed|skipped_no_space|disabled`), `db_backup_last_success_at`, `db_backup_last_error`, `db_backup_last_path`, `db_backup_last_size_bytes`, `db_backup_last_duration_ms`, `db_backup_last_trigger` (`scheduled|boot_pre_migration`), plus `db_backup_first_attempt_at` (staleness reference for never-succeeded installs) and `db_backup_last_staleness_alarm_at`.
