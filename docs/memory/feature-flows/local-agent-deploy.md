...
9. **Merge Credentials + MCP Credential-Gap Warnings** (#950)
   ...
   - **Since #1900** the subsequent agent-create staging resolves this template's `.mcp.json` from the deploy-local directory itself (`crud._resolve_local_template_dir`), not from the template.yaml's `name:` field. Two consequences: a hostile `name:` can no longer read another tenant's `.mcp.json` into the new agent, and a deploy-local template that declares `credentials.mcp_servers` now genuinely receives `${VAR}` substitution (the old lookup searched the *curated* catalog and always missed). The substituted file overwrites the archive's raw copy — `startup.sh` copies `/generated-creds/.mcp.json` after the template-copy block and unconditionally. See [template-processing.md](template-processing.md).
