...
## Tests
- **Backend** (`tests/unit/test_1296_reminders.py`, 89): migration + live-`select()` tables.py accessor guard, the three trigger constants, `ReminderCreate` XOR, `derive_reminder_key` raw-input stability, db round-trip / tenant-scope / cancel CAS states, retention (terminal pruned, pending+firing kept), `reminder_service` bounds (min/max window, timeout>cap, pending 429, daily 429), router self-gate ATTACK (sibling 403, connector 403), tenant-scope 404, idempotency edges (dup replay, in-flight 409, cancel-then-recreate-fresh), cancel outcomes, list default-pending. **#1831 `fire_at` delegation** (56 of the 89): the seven measured mid-string-`Z` inputs rejected (each a live 500 before the fix), `fire_at`/`raw_fire_spec()` byte-identity (the Invariant #18 no-canonicalization guard), validator⟹parser one-directionally over a fixed Hypothesis-free corpus, the deliberate uppercase-`Z`-separator loosening pinned as intended, the XOR guard proven load-bearing against `parse_iso_timestamp(None)`, and rejection landing before `idempotency_service.begin()` (no stranded `in_flight` → no 24h 409 wedge).
- **Property** (`tests/unit/test_1771b_timestamp_helpers_properties.py::test_P7_…`): validator-acceptance ⟹ parser-can-parse. Shipped by #1826 as a strict xfail documenting the divergence; #1831 makes it hold **by construction** and it now passes. A pass is near-definitional, so non-vacuity is the thing to police: the reported ~21% acceptance is a rate, not coverage (measured, every accepted draw is the strategy's degenerate trailing-`Z` tail), and an accepted `@example` is pinned as the mechanical floor so a strategy edit cannot drop it to a green 0%. The deterministic regression guard is `test_1296_reminders.py`, not this property.
