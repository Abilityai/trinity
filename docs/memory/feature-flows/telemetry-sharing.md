...
> **Status**: ✅ Implemented — ent#12 (consent + egress + backfill, v0.8.5) and ent#437 PR1 (reachable ask, share id, enforced schema v2, outcome mix, send log, delivery that survives a missing receiver), and the benchmark read wired to the live receiver (ent#190, receiver live since 2026-09-04); the heartbeat's cadence anchored on the persisted stamp instead of process age (#2618); every send-log entry and the last-shared stamp record the origin they went to, and the receiver sentence is decided from that record (#2571).
...
share_now ── gates: TELEMETRY_SHARING_ENABLED/DO_NOT_TRACK AND stored consent
          ...
          ── _record_send({sent_at, host (the origin it went to, #2571), ok, http_status|error class, backfill, window_days, payload})  last 5
          ── 2xx ⇒ last_shared_host, then last_shared_at (+ backfill_delivered_at on a backfill)
```
...
- **Warm is spent only when seen**: `warmShown` is persisted by an `IntersectionObserver` on the card root (`FinishSetupCard.vue:271-313`), not when `consentVisible` flips. The Dashboard renders one onboarding card at a time and a card hidden by that rule still mounts, so marking it on the predicate burned the one re-ask while it sat behind the hardening guide. An observer rather than a `display` check because the card surfaces when a *sibling* is dismissed, which changes no state this component can watch. No `IntersectionObserver` → nothing is persisted.
...
1. **Recent sends** keeps the last five attempts — successes and failures, with the HTTP status or the exception *class* (never `str(e)`) — behind the admin panel. Every entry records the **origin it was posted to** (`host`: scheme + host + port after `strip_url_credentials`, never path, query or userinfo — stamped in `_record_send`, so a refused payload or a pre-POST failure carries the origin it was aimed at), and the last-shared stamp records the origin that acknowledged it (`telemetry_sharing_last_shared_host`), so "Last delivered <date>" says to whom (#2571). The receiver sentence is decided from that record, never from the URL configured at read time — the configured value is a boot-time constant, so after "override → 200 → restore the default" the two disagree, which used to read a test sink's acknowledgement as the hosted receiver's. A 404 recorded against the default origin is worded as a 404 at the default address; against any other origin as that receiver answering 404 (`receiver_hint`). The status also carries `receiver_host`, `configured_host` and `receiver_mismatch` (both known and different — an entry written before #2571, or a corrupt value, is "unknown" and never claims a mismatch), and the panel then says plainly that the newest send went elsewhere and where the next one goes. `share_url` is scrubbed of userinfo, query and fragment before it reaches the panel; rows read "to <origin>", and a `failed` attempt never says "answered". What a destination change does NOT do — start a new delivery episode (the stamp and the backfill marker still count a sink's 2xx) — is deferred.
...
All under the `telemetry_sharing_` prefix: `enabled`, `consent_at`, `backfill_days`, `last_shared_at` (ent#12) plus `id`, `dismissed_at`, `first_value_at`, `backfill_delivered_at`, `recent_sends` (ent#437), and `last_shared_host` (#2571 — the origin that acknowledged the stamp; deleting it alone makes "Last delivered" read "to an unknown receiver", the safe direction). The generic `PUT /api/settings/{key}` refuses the whole family (the dedicated routes are the only writers); the generic `DELETE` stays admin-gated and **open** for it by design — it is the reset path, and every deletion moves toward off / ask again / re-mint — or, for `last_shared_at`, one re-share: since #2618 an empty stamp is due at the next wake, so that DELETE costs one bounded heartbeat within 10–20 minutes, consent still gating. Reset the ask on a dev box: `DELETE /api/settings/telemetry_sharing_dismissed_at`.
...
- `src/frontend/tests/unit/telemetryConsent.spec.js` — the visibility matrix, the warm-once rule, the copy's claims (says anonymous / off by default / reversible / last 30 days; never "traceable" or "secure"), the receiver wording (names the recorded origin, never the configured URL; the mismatch sentence names both origins and what happens next, with sharing on or off; legacy rows read as an unknown receiver; never "null"), snooze persistence under blocked storage.
...
- `tests/unit/test_2669_minting_accessor_callers.py` — the static caller guard (#2669): every use of `get_or_create_installation_id`, `get_or_mint_sharing_id` and `get_instance_label` sits at an allowlisted `(path, qualname, name)` with its reason (an aliased or re-bound call is a hit at the call); the identity keys are written only by their home modules; the read twin is pinned to exist; a token-bearing file that cannot be parsed fails closed naming the interpreter; every spelling family, the scope attribution and the verdict in both directions are driven through the same scan the live tree uses. OSS tree only — the private twin is trinity-enterprise#575.
...
Feature-usage / click-through coverage (PR2, child issue); the error-class taxonomy (ent#418); an edition-differentiated ask (ent#496, unblocked by ent#190); a destination change starting a new delivery episode — the stamp and the backfill marker honoured only for the recorded origin — and the private benchmark read using `last_shared_host` instead of hedging (both deferred from #2571); fleet-composition tallies in the benchmark card once a `ready` answer is observable; `main.py` adopting `utils/app_version.py` (debt inbox `2026-09-03-main-version-resolver-adopt-util`).
