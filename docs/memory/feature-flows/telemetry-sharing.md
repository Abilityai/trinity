...
share_now ── gates: TELEMETRY_SHARING_ENABLED/DO_NOT_TRACK AND stored consent
          ...
          ── _record_send({sent_at, destination, ok, http_status|error class, backfill, window_days, payload})  last 5
...
1. **Recent sends** keeps the last five attempts — successes and failures, with the HTTP status or the exception *class* (never `str(e)`) — behind the admin panel. A 404 from the default URL is worded as a 404 at the default address; from an overridden `TELEMETRY_SHARING_URL` as that receiver answering 404 (`receiver_hint`). Since #2571 each entry carries its `destination` — the origin (`scheme://host[:port]`) of `TELEMETRY_SHARING_URL` at send time, credential-stripped through `strip_url_credentials`, never the path or query — and the status read compares the newest entry's destination with the currently configured one (`configured_destination` / `receiver_destination` / `destination_changed`), so the receiver sentence names the host that answered and says when the configured address has since moved; the 404 wording is judged by where that send went, not by today's URL. Entries from before the key render as `unknown host`, with no migration.
...
- `src/frontend/tests/unit/telemetryConsent.spec.js` — the visibility matrix, the warm-once rule, the copy's claims (says anonymous / off by default / reversible / last 30 days; never "traceable" or "secure"), the receiver wording, snooze persistence under blocked storage, the host that answered / changed-address clause / unrecorded destination, and a source-structure guard over the panel's per-row destination (#2571).
...
Feature-usage / click-through coverage (PR2, child issue); the error-class taxonomy (ent#418); an edition-differentiated ask (ent#496, unblocked by ent#190); fleet-composition tallies in the benchmark card once a `ready` answer is observable; `main.py` adopting `utils/app_version.py` (debt inbox `2026-09-03-main-version-resolver-adopt-util`).
