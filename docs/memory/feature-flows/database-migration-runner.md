...
- **Alembic graph shape is a separate concern with a separate guard (#2068)** — this doc covers the SQLite runner's *execution* safety; nothing here protects the shape of the Alembic **graph** on the PG side. Two revisions declaring the same `down_revision` give a version-line two heads, and `upgrade head` is singular: it resolves its target before applying anything, so it applies **zero** revisions and raises `CommandError: Multiple head revisions are present`. Symmetry is worth knowing when triaging — on the OSS line that aborts `init_database()` (loud, `/health` 503, same surface as above); on the enterprise line the identical fault is swallowed by the registration seam and the instance degrades to OSS-only with every signal green. Guarded pre-merge by `scripts/ci/check_alembic_heads.py` (unconditional step in `schema-parity`, plus the same assertion at the private track's own branch point) and post-merge by the `deploy-dev` registration boot-log assertion. **That pre-merge run is fresh only at PR-event time (#2533)**, which is worth knowing when triaging a fork that shipped green: `schema-parity` runs on `pull_request`, and GitHub recomputes `refs/pull/N/merge` when the base advances without re-triggering workflows — so the last green run can describe a base that no longer exists while the PR reads *mergeable, all checks green*. `.github/workflows/alembic-head-watch.yml` re-runs the same script, unmodified, over an in-memory `git merge-tree` of every open migration PR against the live `dev` tip on each push to `dev`, publishing an **advisory** `alembic-head-watch` commit status plus one sticky comment; it narrows the window rather than closing the class, and it can never block a merge, so a green PR still needs the usual eyes. No flow doc of its own: the mechanism is one stdlib script plus a workflow around it, described in full in Invariant #3 and `requirements/infrastructure.md` §8.11.
