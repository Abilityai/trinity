...
- `results_recovered: int` - `running` rows finalized from the terminal the agent RETAINED after the backend lost the held-open connection — Issue #2944. A recovery of the result, not a fabricated failure: summed into `total`
- `result_claims_deferred: int` / `result_probe_deferred: int` - #2944 observability, NOT in `total`: a retained record too young to claim (a live dispatcher may still write it) / an agent whose `/result` could not be asked (unreachable, 5xx, over-cap — withheld for the cycle)
- `total` property: Sum of the recovery/prune counters (the observability counters above are excluded)
...
| `_orphan_error_message(agent_name, agent_reports_pending, no_retained_result=False)` | The honest string: `Execution not tracked by agent '<name>' (not running, not pending, not recently completed) and no live backend dispatcher (not parked in this worker, no cross-worker marker) — recovered by watchdog` — `not pending` is omitted when the image did not report `pending_ids`. Replaces "Execution completed on agent but status not reported", which asserted a completion for a row the agent never received |
| `_KnownIds.recently_completed` (#2944) | The `recently_completed_ids` SUBSET of the known set — a turn the agent has FINISHED, not one it is running. Told apart because a finished row whose dispatcher is gone has a **result to claim**, never a timeout to enforce |
| `_fetch_retained_result(get, agent_name, id)` (#2944) | `GET /api/executions/{id}/result` → `("found", record)` / `("none")` (coded 404 `{"detail":{"code":"no_retained_result"}}` — asked, nothing there) / `("old_image")` (bare FastAPI 404, non-record 200, stub leak — no such route: the pre-#2944 path runs unchanged) / `("error")` (unreachable, 5xx, bad JSON, over the callback's 4 M-char / 16 MB caps — withhold, #2196). `get` is a callable so the periodic sweep (raw httpx + `build_agent_auth_headers`) and the boot path (`get_agent_client`) share one classification |
| `_claim_retained_result(agent, id, record, *, report/stats, broadcast)` (#2944) | `"claimed"` / `"deferred"` / `"lost"` / `"error"`. **Grace-gated**: a record younger than `RETAINED_RESULT_CLAIM_GRACE_SECONDS` (180 s) is deferred — a LIVE dispatcher is not reliably `alive` between its POST returning and its terminal CAS (`track_inflight_dispatch` pops the in-process entry the instant the call returns; the cross-worker marker delete flushes on the 15 s tick; the SUB-003 / #678 retries reuse the id with an uncovered gap), and claiming inside that window would beat the caller to the CAS (empty RECONCILED response for a healthy turn, slot released twice). Then `ExecutionResultEnvelope` → `terminal_from_callback_payload` (the callback's OWN mapper, `services/execution_envelope.py`) → SUCCESS responses prefixed with `_RECOVERED_NOTICE` (the #1870/#2127 precedent: the row is SUCCESS, the operator can tell it was salvaged, and the chat thread may lack the reply) → `apply_result(agent, env, activity_id=db.get_open_activity_id_for_execution(id, include_failed=True), breaker_enabled=dispatch_breaker_active(agent), release_slot=True)` — **exactly the result-callback's call**, so a claimed turn finalizes the way a callback-delivered one does (activity close, capacity, breaker, terminal event; #1804 class avoided by the activity id). `lost` = the CAS went to a live dispatcher / a cancel / another worker's sweep (compared by `.value`, the #1085 enum quirk): nothing counted, nothing broadcast — every uvicorn worker runs this loop |
...
5. **Collect first, read once** (#2433): every row absent from its (reachable) agent's known set — and, since #2944, every row in its `recently_completed` subset — is collected as a candidate, then `_inflight_verdict_map(candidates)` runs ONCE — one Redis `MGET` per sweep, never per row
...
| Yes | No | `absent` | ≥ 60s | **ASK FOR THE RESULT FIRST (#2944)**: `found` → **CLAIM** (`results_recovered += 1`, `result_recovered` event; `deferred`/`error` → withhold, `result_probe_deferred += 1`); `none` → **ORPHAN RECOVERY** with `_orphan_error_message(…, no_retained_result=True)`; `old_image` → **ORPHAN RECOVERY** with the #2433 string unchanged; `error` → **WITHHELD** this cycle (`result_probe_deferred += 1`; the 120-min stale sweep bounds it) |
| Yes | Yes, in `recently_completed` (#2944) | `alive` / `unknown`-withheld | — | **CONFIRMED RUNNING** — the #921 race: the live dispatcher writes the terminal itself |
| Yes | Yes, in `recently_completed` (#2944) | `absent` | any (evaluated BEFORE the timeout row below — a finished row past its timeout has a result to claim, not a process to terminate; the terminate would 404 and defer it two hours) | `found` + grace elapsed → **CLAIM**; `found` + young → **CONFIRMED RUNNING** (`result_claims_deferred += 1`); `none` / `old_image` → **CONFIRMED RUNNING** (the pre-#2944 skip); `error` → **CONFIRMED RUNNING** (`result_probe_deferred += 1`). A claimed id is NOT added to `confirmed_running` |
...
Module-level coroutine, awaited once from the `main.py` lifespan on boot (before `mark_startup_recovery_complete()`); it has no `CleanupReport` and returns `{recovered, recovered_results, still_running, skipped_grace, cas_lost, errors, redis_slots_reclaimed, activities_closed}`. Two passes:
...
1. **SQL → Redis** — `db.get_running_executions()` grouped by agent:
   ...
   - Container up → `registry_ids = _extract_agent_known_ids(GET /api/executions/running)` (5s timeout; unreachable → empty set + WARNING). **#2433:** the agent's absent-from-registry, non-grace rows are read with ONE `_inflight_verdict_map` call (one `MGET` per agent); `_inflight_skip(verdict, _row_age_seconds(row))` → counted `still_running` and logged at INFO (`[Recovery] <id> on '<agent>' is owned by a live (or unverifiable) backend dispatcher in another worker — left running (#2433)`); otherwise `_recover_execution`. The worker that just booted has an EMPTY in-process registry, so the cross-worker marker is the only signal that the other uvicorn worker still holds the parked coroutine. After a FULL restart every marker lapses within `INFLIGHT_MARKER_TTL_SECONDS` (60s), so such a row waits at most one periodic sweep instead of being recovered here — the #408 dead-coroutine class is unchanged. **#2944:** a row in the agent's `recently_completed` subset is claimed HERE (this worker has no dispatcher for it by construction; the cross-worker marker is still consulted, and the 180 s grace still applies because a worker-recycle boot runs beside live workers) via `_fetch_retained_result` (`get_agent_client(...).get`) + `_claim_retained_result(..., stats=stats)` → `recovered_results`; not claimable → `still_running` (the periodic sweep retries). An absent-everywhere row asks for a retained result before `_recover_execution` — `found` → claim; `lost`/`deferred`/`error` → `still_running`; `none`/`old_image` → recovered as before. An agent whose registry could not be reached keeps the pre-#2944 contract (recovered, `test_1811` parity) — no second probe against a silent agent. Rows genuinely running on the agent stay `still_running`; the periodic sweep claims their terminal on the first cycle after it lands — that is the "re-attach", no separate poller.
...
`tests/unit/test_2944_watchdog_claims_result.py` — the reported case claimed as SUCCESS (envelope, notice, the callback's exact `apply_result` inputs, `confirmed_running` exclusion, event), the grace gate (a 30 s-old record is deferred; grace > marker TTL + tick), `alive`/`unknown` never probed, a completed row past its timeout claimed not terminated, coded-404 → orphan string gains "no retained result", bare-404 / non-record 200 / stub leak → the #2433 string byte-identical, transport error / 5xx / bad JSON / over-cap → withheld, lost CAS → nothing counted, the shared mapper's three-way + auth-never-cancel rules, cap parity with the callback endpoint, and the boot path (claimed immediately; fresh → left for the sweep; old image → recovered as before). Negative control: 27 of these fail on the pre-#2944 `cleanup_service.py` + `chat.py`. Agent side: `tests/unit/test_2944_retained_results.py`.
