...
- `public_chat_messages.sender_email` (nullable) — the verified speaker. `summarize_user_memory_background` reads `get_recent_public_chat_messages(session_id, sender_email=user_email)`, so a shared thread never feeds one user's turns into another user's durable, re-injected MEM-001 memory. The web public-chat path stamps `sender_email` on its user turn too, so the filter is a no-op there.
